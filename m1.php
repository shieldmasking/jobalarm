...
//$searchAdd2 =" AND ((j.postId ={$acctAdd} AND j.brand !={$brandOrig}) OR (j.postId !={$acctAdd} AND a.brandId!={$industry}))";
$searchAdd2 =" AND j.postId !={$acctAdd} AND j.jobCats !={$industry}";
$brandAdd = " AND j.postId=".$acctAdd." AND j.brand=".$brandOrig;
}else if (intval($accType)==5){
...
$dbData = Config::get('db')->get_results("SELECT SQL_CALC_FOUND_ROWS j.*, j.id as jobId, a.status as acctStatus, a.brandId as industry, s.userPic as userPic, s.storeBrand, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist 
	FROM job j
	    ...
	    inner join cities_extended ce on ce.zip = j.zipCode
	where 
        (3963*ACOS(SIN(({$lat}+.000001)/57.2958)*SIN(ce.latitude/57.2958)+ COS(({$lat}+.000001)/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-(({$lon}+.000001)/57.2958))))<=$zipDist and j.status>0 and a.status >=0$searchAdd2$searchAdd 
		$groupBy2 
...
<div class="form-group m-form__group" id="cityadd" hidden>
...
foreach($jobsList as $tweet) {
    ...
    if ($featuring && !isset($tweet['featured'])) {
        $featuring = false;
        //echo '<div style="height:3px;margin:25px 0;width:100%;border-bottom:3px solid #87A9C7"></div><ins class="adsbygoogle" style="display:inline-block;width:280px;height:60px" data-ad-client="ca-pub-2545585330917467" data-ad-slot="1689787287"></ins><div style="height:3px;margin:25px 0;width:100%;border-bottom:3px solid #87A9C7"></div>';
