...
if (isset($_POST['sitelogin'])) {
    ...
    if (count($dbData) > 0) {
        $_SESSION['user'] = $dbData[0];
        $data = array('lastlogin_date'=>date('Y-m-d H:i:s'));
        $where = array('id'=>$dbData[0]['id']);
        Config::get('db')->update('user',$data,$where,1);
        header('location: quickpost.php');
        exit();
    } else {
        $login_error = 1;
	 }
...
<div class="page-container login" style="margin-bottom:30px">
    ...
    <div class="content">
	    ...
	    <form class="login-form" action="login2.php" method="post">
		    ...
		    <h3 class="form-title">Manager Log In</h3>
		    ...
		    <div class="form-group">
			    ...
			    <input class="form-control placeholder-no-fix" type="text" autocomplete="off" placeholder="Account ID" name="username"/>
		    </div>
		    <div class="form-group">
