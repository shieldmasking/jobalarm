...
function GenerateReports() {
	$idsearch = isset($_REQUEST['idsearch']) && ($_REQUEST['idsearch'] != 0) ? $_REQUEST['idsearch'] : '';
	$categoryId = isset($_REQUEST['categoryId']) && ($_REQUEST['categoryId'] != 0) ? $_REQUEST['categoryId'] : '';
	...
	if(intval($role)<=7 && !$idsearch){
		$searchAdd = " AND d.id in (SELECT `deptId` from `productiveDeptXref` where `userId` ={$userId} group by `deptId` order by `deptId`)";
	}else if(intval($role)>=8 && !$idsearch){
		$searchAdd = '';
	}else{
		$searchAdd = " AND d.id ={$deptNum}";
	}
	...
	$dbDept = Config::get('db')->get_results("SELECT d.*, a.payPeriod, a.payPeriodFirst from `ProductiveDept` d LEFT JOIN `productiveNewData` as n on n.deptId = d.id left join `productiveAccount` as a on a.id = d.accountId where d.accountId = {$accountId}$searchAdd$searchAdd2 group by d.id order by d.dept ASC");
	$outData = array();
	...
	$canData = Config::get('db')->get_results("
        ...
        WHERE
            n.accountId={$accountId} AND n.userId>0 AND n.deptId={$deptId} AND n.whpPlan !=1
        AND 
	...
	$canGraphData = Config::get('db')->get_results("
        ...
        FROM
            `productiveNewData` n 
        WHERE
            n.accountId={$accountId} AND n.deptId={$deptId} AND n.whpPlan !=1 
		AND
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}' 
    ...
    if (count($canGraphData) > 0) {
        foreach ($canGraphData as $graphItem) {
            ...
            $smsSentGraphData[] = $graphItem['procedures'];
			$laborGraphLabels[] = $graphItem['groupDate'];
            $laborGraphData[] = $graphItem['hours'];
			$postpartumGraphLabels[] = $graphItem['groupDate'];
            $postpartumGraphData[] = $graphItem['whp'];
        }
    }
	...
	$canData = Config::get('db')->get_results("
        ...
        SELECT 
           SUM(n.nvariance) AS canTotal, AVG(n.budgetValue) AS laborTotal, d.prodMeasure, AVG(n.actualWHP) AS postTotal, AVG(n.actualWHP - d.hppd) as hppd
        FROM
            `productiveNewData` n 
        LEFT JOIN 
			`ProductiveDept` as d on d.id = n.deptId 
	...
	$canData = Config::get('db')->get_results("
        ...
        LEFT JOIN 
			`ProductiveDept` as d on d.id = n.deptId 
        WHERE
            n.accountId={$accountId} AND n.deptId={$deptId} 
        AND 
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}'            
    ");	
	...
	$escalationData = Config::get('db')->get_results("
      SELECT 
        ...
        WHERE
            a.accountId={$accountId} AND a.deptId ={$deptId} 
        AND 
    ...
    $smsData = Config::get('db')->get_results("
      SELECT 
		...
		LEFT JOIN 
			`ProductiveDept` as d on d.id=n.deptId 
        WHERE
            n.accountId={$accountId} AND n.whpPlan =1 AND n.userId>0 AND n.deptId={$deptId}
        AND 
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}'
		ORDER BY 
	...
	$canData = Config::get('db')->get_results("
        ...
        WHERE
            n.accountId={$accountId} AND n.userId>0 AND n.deptId={$deptId} 
        AND 
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}' 
		ORDER BY 
	...
	AND n.userId>0 AND n.deptId={$deptId} 
	...
	$postpartumData = Config::get('db')->get_results("
      SELECT 
        ...
        FROM
            `productiveNewData` n 
        WHERE
            n.accountId={$accountId} AND n.userId AND n.deptId={$deptId} 
		AND 
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}' 
    ...
    $churnData = Config::get('db')->get_results("
      SELECT 
        ...
        FROM
            `productiveNewData` n 
        WHERE
            n.accountId={$accountId} AND (n.admits >0 OR n.transfers >0 OR n.discharges >0) AND n.deptId={$deptId} 
        AND 
            n.dayDate >= '{$startDate}' AND 
            n.dayDate <= '{$endDate}'
		ORDER BY 
...
function GetAllUsers() {
	...
	if(intval($role)<8){
		$searchAdd = " AND u.first_name !='' and u.role <={$role} and x.deptId in (SELECT `deptId` from `productiveDeptXref` where `userId`={$userId} and `primaryUnit`>0)";
	}else if(intval($role)==9){
		$searchAdd = " AND u.first_name !='' and u.role >=8 and u.role <=9";
	}else{
		$searchAdd = '';
	}
		...
		u.*, IFNULL(x.deptId,0) as depNum, x.textAlerts, x.userId, count(x.deptId) as deptCount, d.dept as depName, r.role as userRole FROM `productiveUser` u
        ...
        WHERE u.accountId={$accountId} and u.active>0$searchAdd
...
function GetUserDetails() {
    ...
    if (intval($deptId)>0 && $deptId) {
        $dbLocation = Config::get('db')->get_results("select u.*, x.textAlerts as Alerts, x.userId, x.deptId, d.dept as depName, r.id as roleId, r.role as roleName from `productiveUser` u LEFT JOIN `productiveDeptXref` as x on x.userId = u.id LEFT JOIN `ProductiveDept` as d on d.id = x.deptId LEFT OUTER JOIN `productiveUserRoles` as r on r.id=u.role where x.userId={$userId} and x.deptId={$deptId} and u.accountId={$accountId}");
        $dbData = $dbLocation[0];
        echo json_encode(array('success' => true,'data'=>$dbData,'user'=>$userRole));
        return true;
    }else{
		$dbLocation = Config::get('db')->get_results("select u.*, u.id as userId, IFNULL(x.deptId,0) as deptId, r.id as roleId, r.role as roleName from `productiveUser` u LEFT OUTER JOIN `productiveDeptXref` as x on x.userId = u.id LEFT OUTER JOIN `productiveUserRoles` as r on r.id=u.role where u.id={$userId} and u.accountId ={$accountId}");
        $dbData = $dbLocation[0];
        echo json_encode(array('success' => true,'data'=>$dbData,'user'=>$userRole));
        return true;
	}
...
function GetNewUnitDetails() {
	...
	$accountId = Config::get('account')['accountId'];
	//echo "store:".$storeId;
    $dbUnits = Config::get('db')->get_results("select x.*, u.role, count(x.deptId) as deptCount FROM `productiveDeptXref` x LEFT OUTER JOIN `productiveUser` as u on u.id = x.userId WHERE x.userId={$userId} and u.accountId ={$accountId} group by x.userId");
	if($dbUnits){
		$dbCount = $dbUnits[0];
			$dcount = array(
...
function AddNewUser() {
	...
	$deptId = (isset($_REQUEST['unit'])) ? $_REQUEST['unit'] : 0;
	$accountId = Config::get('account')['accountId'];
	...
	//$dbUser= Config::get('db')->get_results("select u.*, IFNULL(l.labelName,'ProductiveRN') as labelName, IFNULL(l.login,'') as login, x.deptId, IFNULL(x.primaryUnit,'') as primaryUnit, IFNULL(d.dept,'') as dept FROM `productiveUser` u LEFT OUTER JOIN `productiveDeptXref` as x on x.userId = u.id and x.primaryUnit>0 LEFT OUTER JOIN `ProductiveDept` as d on d.id = x.deptId LEFT JOIN `productiveAccount` as a on a.id = d.accountId LEFT OUTER JOIN `productiveLabel` as l on l.id = a.label where LOWER(u.email)='{$email}'");
	$dbUser= Config::get('db')->get_results("select d.*, IFNULL(u.id,'') as newUserId, IFNULL(u.active,'') as userActive, IFNULL(l.labelName,'ProductiveRN') as labelName, IFNULL(l.login,'') as login, x.deptId, IFNULL(x.primaryUnit,'') as primaryUnit, IFNULL(d.dept,'') as dept FROM `ProductiveDept` d LEFT OUTER JOIN `productiveUser` as u on u.email ='{$email}' LEFT OUTER JOIN `productiveDeptXref` as x on x.userId = u.id and x.primaryUnit>0 LEFT JOIN `productiveAccount` as a on a.id = d.accountId LEFT OUTER JOIN `productiveLabel` as l on l.id = a.label where d.id={$deptId}");
		$userId = intval($dbUser[0]['newUserId']);
		$userActive = intval($dbUser[0]['userActive']);
		$deptName = $dbUser[0]['dept'];
		$labelName = $dbUser[0]['labelName'];
		$login = $dbUser[0]['login'];	
	
	
	if ($userId) {						
		if ($userActive>0 && $deptName) {
		$exists = array(
			'exist'=>true,
			'email'=>$email,
			'dept'=>$deptName,
			...
			$newUser = Config::get('db')->insert('productiveDeptXref',$data);
					...
					$subject = $labelName . " Access";
		$to = $email;
		$url = "https://www.productivern.com/$login";
		$video = "https://recordings.join.me/JMty2BeeyUuuAf-e5gsWwg";
		...
		<tr></tr>
		<tr>
		<td>A $labelName ID has been created for you.  Your ID and temporary password are below.</td>
		</tr>
		<tr></tr>
		<tr>
		<td>ID: $email</td>
		...
		<tr>
		<td>New Password: $pw</td>
		</tr>
		<th></th>
		<tr>
		<td>To login, go to <a href=" . $url . ">$labelName</a></td>
		</tr>
		<tr></tr>
		<tr>
		<td>To see how to submit a Staffing Report on $labelName, click <a href=" . $video . ">here.</a></td>
		</tr>
		<tr></tr>
		<tr>
...
function Forgot() {
	...
	$dbData = Config::get('db')->get_results("select * FROM `productiveUser` where `email`='{$email}'");
...
<html>
<head>
<title>ProductiveRN Password Reset</title>
</head>
<body>
<table>
<tr>
...
<tr></tr>
<tr>
<td>Your Productive RN password has been reset.  Your Temporary password is below.</td>
</tr>
<tr></tr>
<tr>
<td>New Password: $pw</td>
...
<th></th>
<tr>
<td>To login, go to https://www.productivern.com</td>
</tr>
<th></th>
<tr>
<th>Thank you for using ProductiveRN!</th>
</tr>
</table>
<tr></tr><tr></tr>
<tr><p><h5>
...
function UpdateUser() {
   ...
   if (intval($role)==4 && intval($roleOrig)==6) {
		$datax = array(
            'primaryUnit' =>$role,
            'textAlerts' =>0
        );
...
function GetunitGrid() {
    ...
    if ($userId) {
        $dbLocation = Config::get('db')->get_results("SELECT u.*, x.id as Xid, x.textAlerts, x.userId, x.deptId as depNum, r.role as roleName, x.id as Xid, x.primaryUnit, d.dept as deptName FROM `productiveUser` u LEFT OUTER JOIN `productiveDeptXref` as x on x.userId = u.id LEFT JOIN `ProductiveDept` as d on d.id = x.deptId LEFT OUTER JOIN `productiveUserRoles` as r on r.id = x.primaryUnit WHERE u.id={$userId} order by x.primaryUnit DESC, d.dept ASC");
	...
	foreach ($dbLocation as $dbData){
		...
		if(intval($dbData['primaryUnit'])==0 && intval($userRole)<7){
			$primary = " (Unit Assigned)";
			//$action = "<a href=\"#units\">Change in Unit Settings</a>";
			$action = "Change in Unit Settings";
		}else if(intval($dbData['primaryUnit'])==0 && intval($userRole)>=7){
			$primary = " (Unit Assigned)";
...
function GettextConfigure() {
	...
	//echo "store:".$storeId;
    if ($deptId) {
        $dbLocation = Config::get('db')->get_results("SELECT x.*, r.role as roleName, u.last_name, u.first_name, u.role, u.txtactive, u.txtPause FROM `productiveDeptXref` x LEFT OUTER JOIN `productiveUser` as u on u.id = x.userId LEFT JOIN `productiveUserRoles` as r on r.id = u.role WHERE x.deptid={$deptId} and x.primaryUnit=0");
    if ($dbLocation){
	foreach ($dbLocation as $dbData){
		if (intval($dbData['txtPause'])==0 && intval($dbData['textAlerts'])>0 && intval($dbData['txtactive'])>0){
			$Status = "Yes - Active (User receiving Text Alerts)";
...
function GetAllUnits() {
	...
	if(intval($role)<15){
		$searchAdd = " AND d.id in (SELECT `deptId` from `productiveDeptXref` where `userId`={$userId} and `primaryUnit`>5)";
		//$searchAdd = " AND (d.director={$userId} or d.mgrId={$userId})";
	}else{
		$searchAdd = '';
	}
    
        $query = "SELECT SQL_CALC_FOUND_ROWS
		d.*, c.categoryName, d.unitId as number, IFNULL(u.last_name,'') as manager, m.name as prodname, IFNULL(u.first_name,'') as first_name, IFNULL(u.role,0) as role    
        ...
        WHERE d.accountId ={$accountId}$searchAdd 
		GROUP BY d.id ORDER BY d.dept ASC
		";
    ...
    foreach ($dbData as $job) {
		...
		//if (intval($role)==5){
		//$unitLink = '<a href="#configure">' . $name . '</a>';	
		$unitLink = '<a href="javascript:;" onclick="tj.editUnitMgr('.$job['id'].' , '.$role.');return false;" >' . $name . '</a>';	
		//}else{
        //		$unitLink = '<a href="javascript:;" onclick="tj.editUnit('.$job['id'].');return false;" >' . $name . '</a>';
		//}
...
function GetAllAccounts() {
	...
	if(intval($role)==1){
	$searchAdd = "a.active>0";
	}else if(intval($role)>=7){
		$searchAdd = "a.active>0 AND a.id={$accountId}";
	}else{
		$searchAdd = "a.active<0 AND a.id={$accountId}";
	}
...
function GetUnitDetails() {
	...
	$accountId = Config::get('account')['accountId'];
        ...
        $dbUnits = Config::get('db')->get_results("select d.*, d.dept as unitName, IFNULL(u.last_name,'') as last_name, IFNULL(u.id,0) as userId, IFNULL(u.first_name,'') as first_name from `ProductiveDept` d LEFT JOIN `productiveDeptXref` as x on x.deptId = d.id and x.primaryUnit=6 LEFT JOIN `productiveUser` as u on u.id=x.userId and x.primaryUnit=6 where d.accountId ={$accountId} AND d.id={$deptId}");
        $dbData = $dbUnits[0];
		$dbDir = Config::get('db')->get_results("select d.*, d.dept as unitName, IFNULL(u.last_name,'') as last_name2, IFNULL(u.id,0) as userId2, IFNULL(u.first_name,'') as first_name2 from `ProductiveDept` d LEFT JOIN `productiveDeptXref` as x on x.deptId = d.id and x.primaryUnit=7 LEFT JOIN `productiveUser` as u on u.id=x.userId and x.primaryUnit=7 where d.accountId ={$accountId} AND d.id={$deptId}");
        $dbdirector = $dbDir[0];
...
function AddUnit() {
	...
	$accountId = Config::get('account')['accountId'];
	$userId = Config::get('account')['userId'];	
    ...
    if ($name && $number) {
			...
			$dbXref = Config::get('db')->get_results("select * FROM `productiveShifts` WHERE `shiftsperDay`=4 OR `shiftsperDay`=10");	
			}else{
			$dbXref = Config::get('db')->get_results("select * FROM `productiveShifts` WHERE `shiftsperDay`=6 OR `shiftsperDay`=10");	
			}
			...
			echo json_encode(array('success'=>true,'message'=>'Unit entered successfully.'));
            return true;
	}else{
    echo json_encode(array('success'=>false));
    return false;
...
function GetProdTableData() {
	...
	if(strtotime($startDate)> strtotime('today UTC')){
		$orderBy = " ORDER BY n.dayDate ASC, n.shift DESC";
	}else{
		$orderBy = " ORDER BY n.dayDate DESC, n.shift DESC";
	}
	
    $dbData = Config::get('db')->get_results("SELECT n.*, n.id as nid, a.prodMeasure, a.modalPage, a.hppd, a.dept, a.thresholdHigh, a.thresholdLow, DATE_FORMAT(n.dayDate,'%m/%d/%y') as newDate, IFNULL(c.note,'') as escalationNote, IFNULL(l.escalation,'') as escalationType, IFNULL(c.escalation,0) as escalationId, a.viewPage, s.shift as shiftName, u.first_name, u.last_name FROM `productiveNewData` n LEFT JOIN `productiveShifts` as s on s.id = n.shift LEFT JOIN `ProductiveDept` as a on a.id = n.deptId LEFT OUTER JOIN `productiveUser` as u on u.id = n.userId LEFT OUTER JOIN `productiveEscalations` as c on c.dataId = n.id LEFT OUTER JOIN `productiveAcctEscalations` as l on l.id = c.escalation WHERE a.prodMeasure !=2 AND n.dayDate >='{$startDate}' AND n.dayDate <='{$endDate}' AND n.accountId={$accountId}$searchAdd$userAdd group by n.deptId, n.dayDate, n.shift$orderBy");
    ...
    if($dbData){
		...
		if($nUser==0){
			...
			if(intval($role)>7){
					...
					$shiftDisplay = "<a href=\"#reportView?i=" . $dataId . "\">" . $shiftName . "</a></br><button type=\"button\" id=\"addButtonWHP\" class=\"btn btn-info btn-sm\" onclick=\"tj.editNEW(" . $dataId . "); \">Edit</button>";
				}else if(intval($role)==7 && intval($accountId)!=8 && intval($accountId)!=9){
					$shiftDisplay = "<a href=\"#reportView?i=" . $dataId . "\">" . $shiftName . "</a></br><button type=\"button\" id=\"addButtonWHP\" class=\"btn btn-info btn-sm\" onclick=\"tj.editNEW(" . $dataId . "); \">Edit</button>";
				}else{
					$shiftDisplay = "<a href=\"#reportView?i=" . $dataId . "\">" . $shiftName . "</a></br><button type=\"button\" id=\"addButtonWHP\" class=\"btn btn-danger btn-sm\" onclick=\"tj.clearRecord(" . $dataId . "); \">Del</button>";
				}
				...
				if($escalationId >=1 && $aproductivity >0 && $thresholdHigh >0 && ($aproductivity > $thresholdHigh || $aproductivity < $thresholdLow)){
				$style = "1";
				$note = $newnote. "   ESCALATION: " . strtoupper($escalationType) . " - " . $escalationNote;
				}else if($escalationId >=1 && $aproductivity >0 && $thresholdHigh >0 && ($aproductivity <= $thresholdHigh || $aproductivity >= $thresholdLow)){
				$style = "2";
				$note = $newnote. "   ESCALATION: " . strtoupper($escalationType) . " - " . $escalationNote;
				}else if($escalationId==0 && $aproductivity >0 && $thresholdHigh >0 && ($aproductivity > $thresholdHigh || $aproductivity < $thresholdLow)){
				$style = "1";
				$note = $newnote;
				}else if($escalationId==0 && $aproductivity >0 && $thresholdHigh >0 && ($aproductivity <= $thresholdHigh || $aproductivity >= $thresholdLow)){
				$style = "2";
...
function GetProdTableDataWHP() {
    ...
    if($dbData){
		...
		foreach($dbData as $ante){
			...
			$deptName = $ante['dept'];
			$deptId = $ante['deptId'];
			$dateP = $ante['newDate'] . " PLANNED";
			$dateA = $ante['newDate'] . " ACTUALS";
			$shiftName = $date . " - " . $ante['shiftName'];
			$dateshift = $date.$shift;
			$thresholdHigh = floatval($ante['thresholdHigh']);
			$thresholdLow = floatval($ante['thresholdLow']);			
			...
			if(intval($role)>15 && $whpPlan==1){
				$shiftDisplay = $dateP;
			}else if(intval($role)>15 && $whpPlan!=1){
				$shiftDisplay = $dateA;
			}else if(intval($role)<=15 && $whpPlan==1){
				$shiftDisplay = $dateP . "</br><button type=\"button\" id=\"addButtonWHP\" class=\"btn btn-success btn-sm\" onclick=\"" . $modalPage . "(" . $dataId . "); \">Add</button>";
			...
			if(intval($role)>7 && $whpPlan==1){
				$shiftDisplay = "<a href=\"#reportView?i=" . $dataId . "\">" . $dateP . "</a>";
			}else if(intval($role)>7 && $whpPlan!=1){
			...
			}else if(intval($role)==7 && $whpPlan==1 && (intval($accountId)==9 || intval($accountId)==8) ){
				...
				if($escalationId >=1 && $nUser >0 && ($actualWHP < $thresholdLow || $actualWHP > $thresholdHigh)){
				$style = "1";
				$note = $newnote. "   ESCALATION: " . strtoupper($escalationType) . " - " . stripslashes($escalationNote);
				}else if($escalationId >=1 && $nUser >0 && ($actualWHP >= $thresholdLow || $actualWHP <= $thresholdHigh)){
				$style = "2";
				$note = $newnote. "   ESCALATION: " . strtoupper($escalationType) . " - " . stripslashes($escalationNote);
				}else if($escalationId==0 && $nUser >0 && ($actualWHP < $thresholdLow || $actualWHP > $thresholdHigh)){
				$style = "1";
				$note = $newnote;
				}else if($escalationId==0 && $nUser >0 && ($actualWHP >= $thresholdLow || $actualWHP <= $thresholdHigh)){
				$style = "2";
...
function GetPerformanceTableData() {
	...
	$dbData = Config::get('db')->get_results("SELECT n.id as nid, AVG(NULLIF(n.aproductivity ,0)) as avgprod, SUM(n.nvariance) as runningVar, AVG(n.actualWHP) as avgWHP, n.userId, n.entered, AVG(n.avariance + n.lvariance ) as avgvar, AVG(n.atotal+n.ltotal) as avgpat, count(n.id) as records, u.first_name, u.last_name, d.id as did, d.hppd, d.viewPage, d.dept as depName, d.prodMeasure FROM `productiveNewData` n LEFT JOIN `ProductiveDept` as d on d.id = n.deptId LEFT JOIN `productiveUser` as u on u.id = n.userId WHERE n.userId>0 AND n.dayDate >='{$startDate}' AND n.dayDate <='{$endDate}' AND n.accountId={$accountId} AND n.whpPlan !=1 AND n.userId={$userId} group by n.userId order by `avgprod` DESC");	
	}else if(intval($role)>4 && intval($role)<8 && !$idsearch){
    $dbData = Config::get('db')->get_results("SELECT n.id as nid, AVG(NULLIF(n.aproductivity ,0)) as avgprod, SUM(n.nvariance) as runningVar, AVG(n.actualWHP) as avgWHP, n.userId, n.entered, AVG(n.avariance + n.lvariance ) as avgvar, AVG(n.atotal+n.ltotal) as avgpat, count(n.id) as records, u.first_name, u.last_name, d.id as did, d.hppd, d.viewPage, d.dept as depName, d.prodMeasure FROM `productiveNewData` n LEFT JOIN `ProductiveDept` as d on d.id = n.deptId LEFT JOIN `productiveUser` as u on u.id = n.userId WHERE n.userId>0 AND n.dayDate >='{$startDate}' AND n.dayDate <='{$endDate}' AND n.accountId={$accountId} AND n.whpPlan !=1 AND n.deptId in (SELECT `deptId` from `productiveDeptXref` where `userId`={$userId} and `primaryUnit`>5) group by n.userId order by `avgprod` DESC");
	}else if(intval($role)>=8 && !$idsearch){
    $dbData = Config::get('db')->get_results("SELECT n.id as nid, AVG(NULLIF(n.aproductivity ,0)) as avgprod, SUM(n.nvariance) as runningVar, AVG(n.actualWHP) as avgWHP, n.userId, n.entered, AVG(n.avariance + n.lvariance ) as avgvar, AVG(n.atotal+n.ltotal) as avgpat, count(n.id) as records, u.first_name, u.last_name, d.id as did, d.hppd, d.viewPage, d.dept as depName, d.prodMeasure, a.id FROM `productiveNewData` n LEFT JOIN `ProductiveDept` as d on d.id = n.deptId LEFT JOIN `productiveUser` as u on u.id = n.userId LEFT JOIN `productiveAccount` as a on a.id = d.accountId WHERE n.userId>0 AND n.dayDate >='{$startDate}' AND n.dayDate <='{$endDate}' AND n.whpPlan !=1 AND n.accountId={$accountId} group by n.userId order by `avgprod` DESC");
	...
	}else if(intval($role)>7 && $idsearch){
	$dbData = Config::get('db')->get_results("SELECT n.id as nid, AVG(NULLIF(n.aproductivity ,0)) as avgprod, SUM(n.nvariance) as runningVar, AVG(n.actualWHP) as avgWHP, n.userId, n.entered, AVG(n.avariance + n.lvariance ) as avgvar, AVG(n.atotal+n.ltotal) as avgpat, count(n.id) as records, u.first_name, u.last_name, d.id as did, d.hppd, d.viewPage, d.dept as depName, d.prodMeasure FROM `productiveNewData` n LEFT JOIN `ProductiveDept` as d on d.id = n.deptId LEFT JOIN `productiveUser` as u on u.id = n.userId WHERE n.userId>0 AND n.dayDate >='{$startDate}' AND n.dayDate <='{$endDate}' AND n.accountId={$accountId} AND n.whpPlan !=1 AND n.userId={$idsearch} group by n.dayDate, n.deptId, n.shift order by n.dayDate DESC, n.shift DESC");		
	}else{
	$dbData='';
	}
...
function GetEscalationsTableData() {
	...
	if(intval($role)<=7 && !$idsearch){
		$searchAdd = "AND e.deptId in (SELECT `deptId` from `productiveDeptXref` where `userId`={$userId} and `primaryUnit`>0)";
	}else if(intval($role)>=6 && $idsearch){
		$searchAdd = "AND e.userId={$idsearch} AND e.deptId in (SELECT `deptId` from `productiveDeptXref` where `userId`={$userId} and `primaryUnit`>0)";
	}else{
		$searchAdd = "AND e.userId={$idsearch}";
	}
    ...
    $dbData = Config::get('db')->get_results("SELECT e.*, a.escalation as escalationName, u.first_name, u.last_name, d.dept FROM `productiveEscalations` e LEFT JOIN `productiveAcctEscalations` as a on a.id = e.escalation LEFT JOIN `ProductiveDept` as d on d.id = e.deptId LEFT JOIN `productiveUser` as u on u.id = e.userId WHERE DATE_FORMAT(e.dateSubmitted,'%Y-%m-%d') >='{$startDate}' AND DATE_FORMAT(e.dateSubmitted,'%Y-%m-%d') <='{$endDate}' AND e.accountId={$accountId} and e.escalation !=0 $searchAdd group by e.id order by e.dateSubmitted desc");
...
function GetPoliciesTableData() {
    ...
    $dbData = Config::get('db')->get_results("SELECT p.*, DATE_FORMAT(p.created,'%m/%d/%Y') as createdDate FROM `policies` p order by p.title asc");
    ...
    if($dbData){
		...
		foreach($dbData as $ante){
			...
			$doc = '<a href="javascript:;" onclick="tj.showPolicy('.$ante['id'].')" >' . $ante['title'] . '</a>';
			$created = $ante['createdDate'];
			$title = $ante['title'];
		...
		$outData = array();
		$outData['data'] = $jobArray;
		echo json_encode(array('success' => true,$outData));
		}else {
        $outData = array();
		$jobArray[] = array(
				'title' => '',
...
function UpdateProdTest() {
	...
	$dbData = Config::get('db')->get_results("select n.*, s.value, h.hppd, h.escalations, h.prodMeasure, h.id, h.viewPage, h.dept, h.churn, h.shiftsperDay, h.shift as censusShift from `productiveNewData` n left join `ProductiveDept` as h on h.id=n.deptId left join `productiveShifts` as t on t.id = n.shift left join `productiveAccountXref` as s on s.dept = n.deptId and s.shift = n.shift LEFT OUTER JOIN `productiveDeptXref` as x on x.deptId=n.deptId where n.accountId={$accountId} AND n.id={$dataId} group by n.id");
		...
		$requirednurses = round($highacuity+$medacuity+$lowacuity+$acuityOne+$acuityTwo+$acuitySix);
		...
		$nvariance = $avariance;
			...
			*/
			//echo json_encode(array('pct'=>$pctVariance,'sec'=>$secVariance));
			
			
			
			if($hppd >0 && intval($nursecount)>=1){			
			...
			if (intval($shift) != intval($censusShift)){
				...
				foreach($updateProd as $updateShift){
					...
					$updateantenurses = ($updateantecount + $updatenurseAdd + $updatechargecount + $updateseccount + $updatesitter + $updatetechcount) * 24 * $updatevalue;
					$updateaprod = ($updateantehours / $updateantenurses)*95.5;
					$updateaproductivity = round($updateaprod,1);
						
					$updatedata = array(
						'entered' => $updatenow,
						'aproductivity' => $updateaproductivity,
						'nproductivity' => $updateaproductivity
					);
			...
			$antenurses = (intval($nursecount) + $nurseAdd + floatval($chargecount) + intval($seccount) + intval($sittercount) + intval($techcount)) * 24 / $shiftCount;
			$aprod = ($antehours / $antenurses)*95.5;
			$aproductivity = round($aprod,1);
						
			$acheck = $high+$med+$low+$oneto1+$oneto2+$oneto6;
			if($acheck == $patienttotal){
				$antecheck = true;
			}else{
				$antecheck = false;
		...
		if (intval($patienttotal)>0 || intval($nursecount)>0) {
			$now = time()-21600;		
		    $data = array(
                'chargecount' => $chargecount,
                'techcount' => $techcount,
