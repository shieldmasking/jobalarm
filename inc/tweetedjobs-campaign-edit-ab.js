...
tj.getTweets = function (page) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        method: 'post',
        data: { page: page, cid:tj.campaignId },
        success: function (response) {
            ...
            $.each(response['records'], function (index, data) {
                ...
                $('#assign-list').append(addTweet);
            });
            Metronic.unblockUI('#assign-list');
            $('#assign-list .todo-tasklist-item').click(function () {
                if ($(this).parent().attr('id') == 'tweet-list') {
                    $(this).detach().appendTo('#assign-list');
                } else {
                    $(this).detach().appendTo('#tweet-list');
...
jQuery(document).ready(function () {
    ...
    $('#campaign_to').datepicker({ startDate: '+1d', todayBtn: 'linked', autoclose: true });
