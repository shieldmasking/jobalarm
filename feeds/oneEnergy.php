...
$local_file = '/home/tweetedjobs/feeds/ene.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=452");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/ene.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=ene&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=452 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with One Energy / Zoom Mart at {$desc} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where postId=452 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=452");
