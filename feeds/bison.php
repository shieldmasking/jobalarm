...
$local_file = '/home/tweetedjobs/feeds/bison.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `brand`=291");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/bison.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=wb&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE brand=291 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with White Bison Coffee at {$desc}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where brand=291 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where brand=291");
