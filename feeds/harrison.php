...
$local_file = '/home/tweetedjobs/feeds/harrison.xml';
...
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/harrison.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=HGRP&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$title2,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position at {$brandname} (Harrison Group Hotels) at {$address} in {$city}, {$state}. Mobile Apply Now.";
...
function updatejob(){
	...
	$job_data = array(
				...
				'hashTags' => "JobAlarm",
				'brand' => filter_var($job['brand'], FILTER_SANITIZE_STRING),
