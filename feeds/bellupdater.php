...
$dbBrands = Config::get('db') -> get_results("select x.*, c.entered from `candidateXref` x left join `candidate` as c on c.id = x.candidateId where `brandOrig`=7 and `subscribeDate` > '2017-10-25 00:00:00'");


foreach($dbBrands as $brand) {
	$newZip = $brand['entered'];
	$twitterId = $brand['candidateId'];
	

	$dbData = Config::get('db')-> query("update `candidateXref` set `subscribeDate`='{$newZip}' where `candidateId`={$twitterId}");
