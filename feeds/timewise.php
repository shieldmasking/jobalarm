...
$local_file = '/home/tweetedjobs/feeds/timewise.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=449");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/timewise.xml';
	$dom = new DOMDocument();
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=449 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with {$brandname} at {$desc} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where postId=449 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=449");
