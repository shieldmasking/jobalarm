...
$local_file = '/home/tweetedjobs/feeds/abc.xml';
...
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/abc.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=abci&Enhanced=1');
...
function parsexml2($x){
	...
	while ($z->name === 'item')
	{
	   ...
	   updateDB($node->clientrefnum, $node->city, $node->state, $node->address, $node->externalName, $node->pubDate, $node->applyurl, $node->zip, $node->brandname, $node->title, $node->company);
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$title2,$company){
	...
	/*
	if(strpos($brandname, 'Courtyard')) { 
    $brand = 314;
	}else if(strpos($brandname, 'Springhill')) { 
    $brand = 315;
	}else if(strpos($brandname, 'Home2')) { 
    $brand = 316;
	}else if(strpos($brandname, 'Hilton')) { 
    $brand = 317;
	}else if(strpos($brandname, 'Hampton')) { 
    $brand = 318;
	}else if(strpos($brandname, 'Comfort')) { 
    $brand = 319;
	}else if(strpos($brandname, 'Canopy')) { 
    $brand = 320;
	}else if(strpos($brandname, 'Embassy')) { 
    $brand = 321;
	}else if(strpos($brandname, 'Tru')) { 
    $brand = 322;
	}else{
		*/
	$brand = 313;
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position at {$brandname} (ABC Investments and Management) at {$address} in {$city}, {$state}. Mobile Apply Now.";
