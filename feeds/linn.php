...
$local_file = '/home/tweetedjobs/feeds/linn.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=465");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/linn.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=LIN&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$title){
	...
	if(strpos($title, 'Holiday')) { 
    $brand = 299;
	}else if(strpos($title, 'Tumble')) { 
    $brand = 300;
	}else if(strpos($title, 'Goodyear')) { 
    $brand = 301;
	}else{
	$brand= 298;	
	}
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=465 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$uSql = "update cec_job set text = :text where postId=465 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=465");
