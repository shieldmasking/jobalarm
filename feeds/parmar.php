...
$local_file = '/home/tweetedjobs/feeds/parmar.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=459");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/parmar.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=par&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	if(strpos($brandname, 'Subway')) { 
    $brand = 57;
	}else if(strpos($brandname, 'Arby')) { 
    $brand = 18;
	}else if(strpos($brandname, 'A&W')) { 
    $brand = 304;
	}else{
	$brand= 294;	
	}
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			if(substr($brandname,0,3)==substr($parmar,0,3)){
			$text = "{$title} position with {$company} at {$desc} in {$city}, {$state}. Mobile Apply Now!";
			}else{
			$text = "{$title} position at {$brandname} ({$company}), {$desc} in {$city}, {$state}. Mobile Apply Now!";
			}
			
			
			$uSql = "update cec_job set text = :text where postId=459 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=459");
