...
$dbBrands = Config::get('db') -> get_results("select j.*, c.zip as newZip from `job` j left join `cities_extended` as c on c.city = j.city and c.state_code = j.state where `twitterId` LIKE \"CARROL%\" and `brand`=38 and c.zip !='' group by j.twitterId");
