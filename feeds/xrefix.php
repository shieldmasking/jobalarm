...
$dbBrands = Config::get('db') -> get_results("select c.*, j.title from `clickTrack` c left join `job` as j on j.twitterId = c.trackId where c.title ='' group by `candidateId` order by datetime DESC");


foreach($dbBrands as $brand) {
	//$newVal = $brand['type'];
	$title = $brand['title'];
	$accountId = $brand['brand'];
	$candidateId = $brand['candidateId'];
	

	$dbData = Config::get('db')-> query("update `candidateXref` set `job_type`='{$title}', `subscribeDate`=`subscribeDate` where `brandId`='{$accountId}' AND `candidateId`='{$candidateId}'");
