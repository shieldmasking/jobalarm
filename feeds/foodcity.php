...
$local_file = '/home/tweetedjobs/feeds/foodcity.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=451");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/foodcity.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=kvat&Enhanced=1');
...
function parsexml2($x){
	...
	while ($z->name === 'item')
	{
	   ...
	   updateDB($node->clientrefnum, $node->city, $node->state, $node->address, $node->externalName, $node->pubDate, $node->applyurl, $node->zip, $node->brandname);
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname){
	...
	if(strpos($brandname, 'Low')) { 
    $brand = 306;
	$add = "(Food City)";
	$cat = 70;
	}else if(strpos($brandname, 'Ace')) { 
    $brand = 307;
	$add = "(Food City)";
	$cat = 77;
	}else if(strpos($brandname, 'Stores')) { 
    $brand = 305;
	$add = "(Food City)";
	$cat = 70;
	}else if(strpos($brandname, 'Dollar')) { 
    $brand = 308;
	$add = "(Food City)";
	$cat = 74;
	}else{
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=451 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			try {
		    $stmt3 = $dbh->prepare("insert into cec_job (twitterId,brand,campaignId,city,state,postDate,jobCats,userName,title,urls,postId,zipCode)  
		    	 					values(:ref,:brand,:campaignId,:city,:state,:postdate,:jobCats,:username,:title,:url,:postId,:zipCode)
		    	 					");
			...
			$stmt3->bindParam(':jobCats',$cat, PDO::PARAM_STR);
			...
			$text = "{$title} position with {$brandname} {$add} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where postId=451 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=451");
