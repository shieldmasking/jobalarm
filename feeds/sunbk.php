...
$local_file = '/home/tweetedjobs/feeds/sunbk.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `brand`=38 AND `postId`=458");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/sunbk.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=bks&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with Burger King ({$company}) at {$desc} in {$city}, {$state}. Mobile Apply Now!";
