...
$dbBrands = Config::get('db') -> get_results("SELECT * FROM `clickTrack` where `brand`>=20");
...
foreach($dbBrands as $brand) {
	...
	$job= $brand['jobId'];
	$ctId = $brand['id'];
	$dbjob = Config::get('db') -> get_results("SELECT * FROM `job` WHERE `id`='{$job}'");
	
	if ($dbjob){
	$accountId = $dbjob[0]['postId'];
	$brandId = $dbjob[0]['brand'];
	$jobId = $dbjob[0]['id'];
	$dbData = Config::get('db')-> query("update `clickTrack` set `jobaccountId`={$accountId}, `jobbrandId`={$brandId} where `id`={$ctId}");
	}
}
echo "success";	
