...
$dbBrands = Config::get('db') -> get_results("SELECT x.*, c.zip, c.mobile FROM `candidateXref` x left join `candidate` as c on c.id = x.candidateId WHERE x.brandOrig=42 and c.zip='' and c.mobile like \"425%\"");
...
foreach($dbBrands as $brand) {
	...
	$dbData = Config::get('db')-> query("update `candidateXref` set `accountId`='395' where `id`='{$candidateId}'");
