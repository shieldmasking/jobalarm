...
$dbBrands = Config::get('db') -> get_results("SELECT x.*, c.zip, c.mobile FROM `candidateXref` x left join `candidate` as c on c.id = x.candidateId WHERE x.brandOrig=2 and (c.mobile like \"410%\" or c.mobile like \"443%\") and x.accountId = 0");


foreach($dbBrands as $brand) {
	$newjob = $brand['position'];
	$candidateId = $brand['id'];
	//$twitterId = $brand['twitterId'];
	

	$dbData = Config::get('db')-> query("update `candidateXref` set `accountId`='296' where `id`='{$candidateId}'");
