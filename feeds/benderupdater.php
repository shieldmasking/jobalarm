...
$dbBrands = Config::get('db') -> get_results("SELECT x.*, c.zip, c.mobile FROM `candidateXref` x left join `candidate` as c on c.id = x.candidateId WHERE x.brandOrig=2 and c.mobile like \"931%\" and x.accountId = 0");
...
foreach($dbBrands as $brand) {
	...
	$dbData = Config::get('db')-> query("update `candidateXref` set `accountId`='274' where `id`='{$candidateId}'");
