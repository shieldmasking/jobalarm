...
$local_file = '/home/tweetedjobs/feeds/dqf.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=111");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/dqf.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=dqf&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=111 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			try {
		    $stmt3 = $dbh->prepare("insert into cec_job (twitterId,brand,campaignId,city,state,postDate,userName,title,urls,postId,zipCode,hashTags)  
		    	 					values(:ref,:brand,:campaignId,:city,:state,:postdate,:username,:title,:url,:postId,:zipCode,:hashTags)
		    	 					");
			...
			$stmt3->bindParam(':hashTags',$company, PDO::PARAM_STR);
			
			$stmt3->execute();
			$lastId = $dbh->lastInsertId();
			
			$text = "{$title} position with {$brandname} ({$company}) at {$address} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where postId=111 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=111");
