...
$dbBrands = Config::get('db') -> get_results("select j.*, c.zip as newZip from `job` j left join `cities_extended` as c on c.city = j.city and c.state_code = j.state where `twitterId` LIKE \"WALGREENS%\" and j.zipCode ='00000' and c.zip !='' group by j.twitterId");
