...
$local_file = '/home/tweetedjobs/feeds/columbia.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `brand`=295");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/columbia.xml';
	$dom = new DOMDocument();
	$dom->load('https://recruiting.adp.com/srccsh/public/ws_req_feed.guid?c=1110841&d=RetailExternalCareerSite&f=???&o=true ');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE brand=295 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with Columbia Distributing in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where brand=295 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where brand=295");
	
	foreach($newjobs as $job) {
	//$job_text = "{$job['title']} with Columbia Distributing: {$job['text']}. Mobile Apply Now.";
