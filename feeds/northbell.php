...
$local_file = '/home/tweetedjobs/feeds/northbell.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=461 AND `brand`=1");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/northbell.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=tbl&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with Taco Bell ({$company}) at {$desc} in {$city}, {$state}. Mobile Apply Now!";
