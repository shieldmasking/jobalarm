...
$local_file = '/home/tweetedjobs/feeds/fusion.xml';
...
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/fusion.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=fusion&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$title2,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position at {$brandname} (Fusion Hospitality) at {$address} in {$city}, {$state}. Mobile Apply Now.";
