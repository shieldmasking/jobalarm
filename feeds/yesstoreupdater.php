...
$dbBrands = Config::get('db') -> get_results("select j.*, c.city as newCity, c.zip as newZip from `sms_stores` j left join `cities_extended` as c on c.zip = j.zip where j.brandId=116 and j.city ='none' group by j.id");


foreach($dbBrands as $brand) {
	$newZip = $brand['newCity'];
	$twitterId = $brand['storeNum'];
	

	$dbData = Config::get('db')-> query("update `sms_stores` set `city`='{$newZip}' where `storeNum`='{$twitterId}'");
