...
$local_file = '/home/tweetedjobs/feeds/hodges.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=359");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/hodges.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=hdg&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname){
	...
	if($brandname==$dq){
		$brand = 4;
	}else if($brandname==$kfc){
		$brand = 62;
	}else if($brandname==$tb){
		$brand = 1;
	}else{
		$brand = 4;
	}
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE postId=359 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} with {$brandname} (Hodges Managment) at {$desc} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where postId=359 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=359");
