...
$local_file = '/home/tweetedjobs/feeds/jackson.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `brand`=248");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/jackson.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=jackson&Enhanced=1');
...
function parsexml2($x){
	...
	while ($z->name === 'item')
	{
	   ...
	   updateDB($node->clientrefnum, $node->city, $node->state, $node->address, $node->externalName, $node->pubDate, $node->applyurl, $node->zip, $node->company);
...
function updateDB($ref,$city,$state,$desc,$title,$postDate,$url,$zip,$company){
	...
	$stmt = $dbh->prepare('SELECT twitterId FROM cec_job WHERE brand=248 and twitterid=:ref');
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			//$text = "{$title} at {$desc}. Mobile Apply Now.";
			$text = "{$title} with {$company} at {$desc} in {$city}, {$state}. Mobile Apply Now.";
			
			
			
			$uSql = "update cec_job set text = :text where brand=248 and twitterid = :ref";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where brand=248");
