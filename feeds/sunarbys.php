...
$local_file = '/home/tweetedjobs/feeds/sunarbys.xml';
...
$dbDelete = Config::get('db') -> query("delete from `job` where `postId`=458 AND `brand`=18");
}
	
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/sunarbys.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=drm&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with Arbys ({$company}) at {$desc} in {$city}, {$state}. Mobile Apply Now!";
...
function updatejob(){
	$newjobs = Config::get('db')->get_results("select * from cec_job where postId=458 AND hashTags LIKE 'Turbo%'");
	$t = "ARB";
