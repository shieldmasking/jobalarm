...
$local_file = '/home/tweetedjobs/feeds/northkfc.xml';
...
function getDataFeed(){
	$output_filename = '/home/tweetedjobs/feeds/northkfc.xml';
	$dom = new DOMDocument();
	$dom->load('https://secure.jobappnetwork.com/commoncfmx/rss_feed/rss.cfm?c=kfc&Enhanced=1');
...
function updateDB($ref,$city,$state,$desc1,$title,$postDate,$url,$zip,$brandname,$company){
	...
	$row = $stmt->fetch(PDO::FETCH_ASSOC);
		...
		if($stmt->rowCount() == 0)
		{
			...
			$text = "{$title} position with KFC ({$company}) at {$desc} in {$city}, {$state}. Mobile Apply Now!";
