...
$dbBrands = Config::get('db') -> get_results("select j.*, c.zip as newZip from `job` j left join `cities_extended` as c on c.city = j.city and c.state_code = j.state where j.zipCode='99999' group by j.id");
...
foreach($dbBrands as $brand) {
	...
	$dbData = Config::get('db')-> query("update `job` set `zipCode`='{$newZip}' where `id`='{$twitterId}'");
