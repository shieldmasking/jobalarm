...
function findLocation() {
		...
		$storesDb = Config::get('db')->get_results("select x.*, c.zip, ce.latitude, ce.longitude from `candidateXref` x LEFT JOIN `candidate` as c on c.id = x.candidateId LEFT OUTER JOIN `cities_extended` as ce on ce.zip = c.zip where c.zip !='' and ce.latitude>0 and x.stageId=0 and x.brandOrig=134");
		
		//$storesDb = Config::get('db')->get_results("select * from `cities_extended` where zip={$zip}");;		
		
		
		foreach($storesDb as $stores){
		$lat = $stores['latitude'];
		$lon = $stores['longitude'];
		$brandId = $stores['brandOrig'];
		$candidateId = $stores['candidateId'];
				
		$storeData = Config::get('db')->get_results("select s.*, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist from `sms_stores` s left join `cities_extended` as ce on ce.zip = s.zip where s.brandId={$brandId} order by dist ASC");
		
			if($storeData) {
			$dist = $storeData[0]['dist'];
				
				if(intval($dist)<25 && intval($dist)>=0 && $dist !=NULL){
					$storenum = $storeData[0]['id'];
					Config::get('db')->query("update `candidateXref` set `stageId` ='{$storenum}'  WHERE `candidateId` ='{$candidateId}' AND `brandOrig` ='{$brandId}'");
				}
			}
		}
