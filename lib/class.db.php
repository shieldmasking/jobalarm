...
class DB
{
     ...
     *
     */
    public function upsert( $table, $variables = array(), $ondupvars = array() )
    {
        self::$counter++;
        //Make sure the array isn't empty
        if( empty( $variables ) )
        ...
        foreach( $ondupvars as $field => $value )
        {
            $upsertfields[] = " {$field}='{$value}'";
        }
        $upsertfields = implode(', ', $upsertfields);
        
        
        $sql .= $fields .' VALUES '. $values;

        $sql .= " ON DUPLICATE KEY UPDATE ".$upsertfields;
