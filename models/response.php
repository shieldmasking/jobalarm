...
//var_dump($_REQUEST);
		...
		INNER JOIN (select sms_messages.id,sms_messages.candidateId,MAX(sms_messages.msgDate) maxmsgDate from sms_messages where userId=$userx AND type=0 group by sms_messages.candidateId) latest_messages on latest_messages.candidateId=sms_messages.candidateId and sms_messages.msgDate = latest_messages.maxmsgDate
		WHERE (sms_messages.userId =$userx or sms_messages.userId=0) AND sms_messages.type = 0
        {$whereAdd}
        {$search_add}
