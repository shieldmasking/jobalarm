...
$featuredData = Config::get('db')->get_results("SELECT SQL_CALC_FOUND_ROWS j.*, j.id as jobId, j.postId as featured, a.status as acctStatus, s.userPic as userPic, s.storeBrand, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist 
	...
	order by j.title desc");	
		...
		$featureCount = count($featuredData);
	}
}
...
$currentPageTotal = count($dbData);
//$grandTotal = $totalJobRecords;
				...
				<div class="row">
						  <div class="col-md-12">
                           ...
                           <h4 class="form-section" style="text-align: center"><strong><?php echo $msg; ?></strong></h4></br>
							...
							<?php if($brandDeal): ?>
							<div class="container" style="text-align: center"><span><h4><strong>
							Get <?php echo $brandName; ?><a target="_blank" href="<?php echo $brandDeal; ?>" onClick="tj.appClick(<?php echo $mobile .",". $brandOrig.",". $acctAdd; ?>)"> App/Rewards.</a></strong></h4></span>
							</br></br>
							</div>	
							<?php endif;?>
...
<div id="googlead" <?php echo $hideAdd; ?>>											
...
foreach($jobsList as $tweet) {
	...
	if ($searchAdd){
		$moreLike = '<a href="http://www.jobalarm.biz/m.php?z='.$zipcode.'&a='.$acctAdd.'&m='.$mobile.'&b='.$brandOrig.'">Back</a>';
	}else{
		$moreLike = '<a href="http://www.jobalarm.biz/m.php?z='.$zipcode.'&a='.$acctAdd.'&m='.$mobile.'&b='.$brandOrig.'&s='.$moreSearch.'&p='.$morePriority.'">More jobs like this one</a>';
	}
...
?>	
												...
												<div class="<?php echo $listClass; ?>">
													...
													<div class="todo-tasklist-item-title">
														 <!--<a target="_blank" href="" class="tooltips" data-container="body" data-placement="top" data-original-title="Follow This User"></a> -->  <?php echo $tweet['city'].' '.$tweet['state']; ?>
                                                         <?php if ($dateDiff < 2): ?>
