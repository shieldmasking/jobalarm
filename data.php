...
function send_messages_with_optout2($smsAlex,$numberlist,$keywords) {
    $post = '';
    $key = '';
    $mobile = '';
...
include_once 'inc/class.jatwitter.php';
include_once 'inc/config.php';
	$allemps = isset($_POST['allJobs']) ? $_POST['allJobs'] : 1;
	$alldeals = isset($_POST['allDeals']) ? $_POST['allDeals'] : 0;
	...
	if ($mobile){
		...
		$dbData = Config::get('db')->get_results("select x.*, c.mobile as mobile, s.deal from `candidateXref` x left join `candidate` as c on c.id = x.candidateId left join `sms_brand` as s on s.id = x.brandOrig where c.mobile ='{$mobile}' ORDER BY x.subscribeDate DESC");
		$dbResult = $dbData[0];
		
		if ($dbResult){
		$candidateId = $dbResult['candidateId'];
		$numberList[] = "1" . $dbResult['mobile'];				
		$brandOrig = $dbResult['brandOrig'];
		$shortCode = $dbResult['shortCode'];
		$brandId = $dbResult['brandOrig'];
		$deal = $dbResult['deal'];
		$keyword = $dbResult['keyword'];
		$keywords[] = $dbResult['keyword2'];
		
		if (intval($alldeals)>0 ){
				
		$data = array(
			'promoMktng'=>Config::get('db')->filter($alldeals)
			);
		$where = array('candidateId'=>$candidateId);
		Config::get('db')->update('candidateXref',$data,$where);
		}
		}
		if(intval($alldeals)>0){
		$outmessage = "Welcome to JobAlarm Deals! Here's your first deal: {$deal}  Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply.";
		}else{
			exit();
			}
		...
		$msg .= "<partnerpassword>".$psword."</partnerpassword>";
		$msg .= "<content><![CDATA[" . $outmessage . "]]></content>";
		$msg .= "</message>";
		//echo $msg;
		$messages[] = $msg;
		
		if (intval($shortCode)==58046){
		$sms_result = send_messages_with_optout2($messages,$numberList,$keywords);
		}else{
		$sms_result = send_messages_with_optout($messages,$numberList,$keywords);
		}
		
		$data = array(
				'brandId'=>Config::get('db')->filter($brandOrig),				
				'candidateId'=>Config::get('db')->filter($candidateId),
				'type'=>Config::get('db')->filter($type),
				'message'=>Config::get('db')->filter($outmessage),
...
include_once 'inc/config.php';
	...
	$alldeals = isset($_POST['allDeals']) ? $_POST['allDeals'] : 0;
	$mobile = isset($_POST['mobile']) ? $_POST['mobile'] : false;
	$sbrand = isset($_POST['brand']) ? $_POST['brand'] : false;
	...
	if ($mobile){
		...
		$dbData = Config::get('db')->get_results("select x.*, c.mobile as mobile from candidateXref x left join candidate as c on c.id = x.candidateId where c.mobile ={$mobile} ");
		$dbResult = $dbData[0];
		...
		if ($sbrand){
		$dbBrand = Config::get('db')->get_results("select * from `sms_brand` where id={$brand}");
		$dbStore = $dbBrand[0];
		$brandName = $dbStore['storeBrand'];
		$keyword2 = $dbStore['keyword'];
		$brandOrig = $dbStore['id'];
		$keywords[] = $dbStore['keyword'];
		...
		$data = array(
			'candidateId'=>Config::get('db')->filter($candidateId),
			'promoMktng'=>Config::get('db')->filter($alldeals),
			'promo'=>Config::get('db')->filter($allemps),
			'keyword'=>$keyword,
			'brandId'=>$brandId,
			'keyword2'=>$keyword2,
			'brandOrig'=>$brandOrig
			);
		Config::get('db')->insert('candidateXref',$data);
		
		}else{	
			
		$brandOrig = $dbResult['brandOrig'];
		$brandId = $dbResult['brandId'];
		$keyword = $dbResult['keyword'];
		...
		if ($allemps==2 && $alldeals==2){
		$outmessage = "JobAlarm Jobs & Deals! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else if($allemps==2 && $alldeals <2){
		$outmessage = "Welcome to JobAlarm! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else if($allemps==0 && $alldeals==2){
		$outmessage = "Welcome to JobAlarm Deals! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else if($allemps==0 && $alldeals==1){
		$outmessage = "Welcome to ". $brandName . " Deals! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else if($allemps==1 && $alldeals==0){
		$outmessage = "Welcome to ". $brandName . " Jobs! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else if($allemps==1 && $alldeals>0){
		$outmessage = "". $brandName . " Jobs & Deals! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply. Edit your profile at www.jobalarm.com/login3.php";
		}else{
			exit();
			}
		}
...
function appClick() {
	...
	$accountId = isset($_POST['acct']) ? $_POST['acct'] : 0;
...
include_once 'inc/config.php';
	...
	$mobile1 = isset($_POST['mobile']) ? $_POST['mobile'] : false;
	$mobile = trim($mobile1);
	$numberList = array();
    $keywords = array();
	$messages = array();
	...
	if ($mobile){
		...
		$dbData = Config::get('db')->get_results("select x.*, c.mobile as mobile from `candidateXref` x left join `candidate` as c on c.id = x.candidateId where c.mobile ='$mobile' GROUP BY c.mobile ORDER BY x.subscribeDate DESC ");
		$dbResult = $dbData[0];
		...
		//foreach($dbResult as $k => $v) {			
			...
			$brandOrig = $dbResult['brandOrig'];
			$keywords[] = $dbResult['keyword2'];
		//echo $dbResult['mobile'];
		
		$pin = mt_rand(11111,99999);
		$newPin = md5($pin);
		
		$data = array('pin'=>$newPin);
		$where = array('mobile'=>$mobile);
		Config::get('db')->update('candidate',$data,$where);
		
		$outmessage = "Your new JobAlarm PIN is ". $pin . " To access your account, go to www.jobalarm.com/mobile";
		
		$msg = "";
		$msg .= "";
		$msg .= "<message id=\"".$msgId."\">";
		$msg .= "<partnerpassword>".$psword."</partnerpassword>";
		$msg .= "<content>" . $outmessage . "</content>";
		$msg .= "</message>";
		$messages[] = $msg;
...
include_once 'inc/config.php';
    ...
    $jobId = $_POST['jid'];
    $referrer = (isset($_POST['ref'])) ? $_POST['ref'] :'3';
	$mobile = (isset($_POST['mobile'])) ? $_POST['mobile'] :'';
	//$emobile = openssl_encrypt(filter_var($mobile, FILTER_SANITIZE_STRING), $ciphering, $encryption_key, $options, $encryption_iv);
	
	$candDb = Config::get('db')->get_results("select c.*, x.accountId, x.id as xid, x.brandOrig from `candidate` c LEFT JOIN `candidateXref` as x on x.candidateId = c.id where c.id={$mobile} order by xid desc limit 0,1");
    $job = Config::get('db')->get_results("select j.*, s.storeBrand, a.fullName from `job` j left outer join `sms_brand` as s on s.id=j.brand left outer join `account` as a on a.id=j.postId where j.id={$jobId}");
	$clicked = $job[0];
	if ($candDb){
		$brandOrig = $candDb[0]['brandOrig'];
		$acct = $candDb[0]['accountId'];
	}else{
		$brandOrig = 0;
		$acct = 0;
	}
	...
	if ($clicked) {
	$trackId = $clicked['twitterId'];
	$brand = $clicked['brand'];
	$accountId = $clicked['postId'];
	$title = $clicked['title'];
	$jobbrandName = $clicked['fullName'];
	$clickDay = date('Y-m-d');
    			
	switch($referrer) {
    	case '2' : 
        	Config::get('db')->query("insert into clickTrack (jobId,trackId,facebook,dateDay,mobile,candidateId,title,brand,accountId,jobaccountId,jobbrandId,jobbrandName) values({$jobId},'{$trackId}',1,'{$clickDay}','0',(select `id` from `candidate` where `id`='{$mobile}'),'{$title}',{$brandOrig},{$acct},{$accountId},{$brand},'{$jobbrandName}')");
    	break;
    	case '1' :
        	Config::get('db')->query("insert into clickTrack (jobId,trackId,text,dateDay,mobile,candidateId,title,brand,accountId,jobaccountId,jobbrandId,jobbrandName) values({$jobId},'{$trackId}',1,'{$clickDay}','0',(select `id` from `candidate` where `id`='{$mobile}'),'{$title}',{$brandOrig},{$acct},{$accountId},{$brand},'{$jobbrandName}')");
    	break;
    	case '3' :
        	Config::get('db')->query("insert into clickTrack (jobId,trackId,jobalarm,dateDay,mobile,candidateId,title,brand,accountId,jobaccountId,jobbrandId,jobbrandName) values({$jobId},'{$trackId}',1,'{$clickDay}','0',(select `id` from `candidate` where `id`='{$mobile}'),'{$title}',{$brandOrig},{$acct},{$accountId},{$brand},'{$jobbrandName}')");
    	break;
		default:
        	Config::get('db')->query("insert into clickTrack (jobId,trackId,jobalarm,dateDay,mobile,candidateId,title,brand,accountId,jobaccountId,jobbrandId,jobbrandName) values({$jobId},'{$trackId}',1,'{$clickDay}','0',(select `id` from `candidate` where `id`='{$mobile}'),'{$title}',{$brandOrig},{$acct},{$accountId},{$brand},'{$jobbrandName}')");
    	break;
	...
	Config::get('db')->query("update `candidateXref` set `job_type` ='{$title}' WHERE `candidateId`=(select `id` from `candidate` where `id`='{$mobile}') AND `brandOrig`='{$brand}'");
	}
...
include_once 'inc/config.php';
	...
	//echo "mobile ".$mobile;
 	if ($mobile) {   			
   	Config::get('db')->query("insert into `adTrack` (`linkId`,`candidateId`) values({$referrer},(select `id` from `candidate` where `mobile`='{$mobile}'))");
    }
...
if (isset($_GET['tu'])) {
    $Xid = isset($_POST['cand']) ? $_POST['cand'] : '';
	...
	$query = ("SELECT x.*, c.mobile, g.id as currentGroup from `candidateXref` x left outer join `candidate_group` as g on g.candidateId= x.candidateId and g.accountId= x.accountId left join `candidate` as c on c.id = x.candidateId where x.id={$Xid} group by x.candidateId");
	$dbData = Config::get('db')->get_results($query);
	$brandId = $dbData[0]['brandOrig'];
	$candidateId = $dbData[0]['candidateId'];
	$shortCode = $dbData[0]['shortCode'];
	$keyword = $dbData[0]['keyword2'];
	$cGroup = $dbData[0]['currentGroup'];
	$psword = "J8775bcgEE2065";
	$result = '';
	$type = 1;
	
	if (intval($cGroup) > 0){
		$currentGroup = intval($cGroup);
	}else{
		$currentGroup = '';
	}
	...
	if (count($dbData) > 0) {            
		if($message) {
			foreach($dbData as $k => $v) {			
				...
				$smsMsg = "";
				$smsMsg .= "<message id=\"".$msgId."\">";
				$smsMsg .= "<partnerpassword>".$psword."</partnerpassword>";
				$smsMsg .= "<content>".$message."</content>";
				$smsMsg .= "</message>";
				...
				if (intval($shortCode)==58046){
				$result = send_messages_with_optout2($messages,$numberList,$keywords);
				}else{		
				$result = send_messages_with_optout($messages,$numberList,$keywords);
				}
...
if (isset($_GET['rs'])) {
	...
	$dbData = Config::get('db')->get_results("select x.* from `candidateXref` x left join `candidate` as c on c.id = x.candidateId where x.brandOrig={$brandId}");
    
	if ($brandId==6 || $brandId==19){
	//$data = array('promo'=>0,'promoMktng'=>0);
    $where = array('candidateId'=>$dbData[0]['candidateId']);
    Config::get('db')->delete('candidateXref',$where);
	}else{
	//$data = array('promo'=>0,'promoMktng'=>0);
    $where = array('candidateId'=>$dbData[0]['candidateId'],'brandOrig'=>$brandId);
    Config::get('db')->delete('candidateXref',$where);		
	}
...
if (isset($_GET['gaj'])) {
	...
	$query = "SELECT m.*, s.storeBrand, u.last_name, u.first_name FROM `sms_stores` as m LEFT JOIN `assign_store` as a on a.storeId = m.id LEFT JOIN `users` as u on u.id = a.userId LEFT JOIN `sms_brand` as s on s.id = m.brandId WHERE m.accountId={$accountId} AND m.active > 0 GROUP BY m.id ORDER BY m.st,m.city";
...
if (isset($_GET['gco'])) {
	...
	$brandData = "SELECT * FROM account_brand WHERE accountId=$accountId";
	$brand = $brandData[0]['brandId'];
	...
	foreach ($dbData as $job) {
		...
		if (intval($zip) > 0){
		$candidates = Config::get('db')->get_results("SELECT c.*, x.keyword as keyword FROM `candidate` as c LEFT JOIN candidateXref as x on x.candidateId = c.id LEFT JOIN cities_extended ce on ce.zip = c.zip WHERE (LEFT(c.zip, 1) in ('{$zipOrig}','{$zipHigh}','{$zipLow}')) and ((3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958))))<=$miles and c.active = 1) and (x.promo=1 or x.promo=2) and (x.brandId={$brand}{$search_add})");
		//$candidates = Config::get('db')->get_results("SELECT c.*, x.brandId FROM `candidatecountXref` as c LEFT JOIN candidateXref as x on x.candidateId = c.candidateId LEFT JOIN cities_extended ce on ce.zip = c.zip LEFT JOIN candidate as d on d.id = c.candidateId WHERE ((3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958))))<=$miles and d.active = 1 and d.mobile != '') and (x.brandId={$brand}{$search_add}) group by c.candidateId");
		}
		...
		if(!$candidates){
			...
			$candidateLink = '<a href="http://admin.jobalarm.com/login/smslogin/'.$sessionAccount.'/'.$sessionId.'/'.$zip.'/0">'.$totalCandidates.'</a>';
...
if (isset($_GET['atsx'])) {
	...
	//$query = "SELECT j.*,(c.facebook + c.jobalarm + c.text) as clickCount, count(x.id) as autoCount, s.storeNum, s.zip, b.storeBrand FROM `job` j LEFT JOIN `sms_brand` as b on b.id = j.brand LEFT JOIN `sms_stores` as s on s.brandId = j.brand LEFT JOIN `clicks` as c on c.jobId = j.id LEFT JOIN jobautopostXref as x on x.jobId = j.id where j.brand={$brandId} and j.city ='{$city}' and j.state ='{$state}' group by j.title order by j.title ASC";
	
	IF ($brandId ==9){
	$query = "SELECT j.*,(c.facebook + c.jobalarm + c.text) as clickCount, count(x.id) as autoCount, b.storeBrand FROM `job` j LEFT JOIN `sms_brand` as b on b.id = j.brand LEFT JOIN `clicks` as c on c.trackId = j.twitterId LEFT JOIN jobautopostXref as x on x.trackId = j.twitterId where j.brand={$brandId} and j.city ='{$city}' and j.state ='{$state}' and j.status=1 group by j.title order by j.title ASC";
	}else{
	$query = "SELECT j.*,(c.facebook + c.jobalarm + c.text) as clickCount, count(x.id) as autoCount, b.storeBrand FROM `job` j LEFT JOIN `sms_brand` as b on b.id = j.brand LEFT JOIN `clicks` as c on c.trackId = j.twitterId LEFT JOIN jobautopostXref as x on x.trackId = j.twitterId where j.zipCode={$storezip} and j.brand={$brandId} and j.postId=0 and j.status=1 group by j.id order by j.id DESC";
	}	
...
if (isset($_GET['ug'])) {
	...
	$query = "SELECT u.*, COUNT(a.id) as storeCount, b.storeBrand, s.brandId from users u LEFT JOIN assign_brand as ab on ab.userId = u.id LEFT JOIN sms_brand as b on b.id = ab.brandId LEFT OUTER JOIN `assign_store` as a on a.userId = u.id LEFT OUTER JOIN sms_stores as s on s.id = a.storeId WHERE u.accountId ={$account} and u.status =1 GROUP BY u.id order by u.last_name ASC";
	$dbData = Config::get('db') -> get_results($query);
	$query = "SELECT FOUND_ROWS() AS found_rows;";
	$countData = Config::get('db') -> get_results($query);
	$total = $countData[0]['found_rows'];
	...
	foreach ($dbData as $job) {
		...
		$storeButton = "";
		$storeButton .= "<a class=\"btn btn-sm blue\" onclick=\"tj.alex.userxGrid(" . $job['id'] . ");\">Locations </a><a class=\"btn btn-sm green\">Access </a><a class=\"btn btn-sm red\" onclick=\"tj.deleteUser(".$job['id'].");\">Remove </a>";
...
if (isset($_GET['nu'])) {
	...
	//$query = "SELECT u.*, b.storeBrand, COUNT(s.id) as storeCount, s.brandId from users u LEFT JOIN sms_stores as s on s.userId = u.id LEFT JOIN sms_brand as b on b.id = s.brandId WHERE u.accountId ={$account} and u.status =1 GROUP BY u.id ORDER BY u.last_name ASC";
	$query = "SELECT x . * , c.first_name, c.last_name, c.zip, c.entered, c.mobile, j.title, a.brandId2
	...
	foreach ($dbData as $job) {
		...
		$jobArrayTemp[] = $job['mobile'];
		
		if ($job['brandId2']==1){		
		$clickpay = $clickpay + .50;
		}
		
		if (!$job['keyword2']){		
		$keyword = $job['keyword'];
		}else{
		$keyword = $job['keyword2'];
		}
		
		$jobArrayTemp[] = $keyword;
		$jobArrayTemp[] = $clickpay;
		//$jobArrayTemp[] = $job['promoMktng'];
		$jobArrayTemp[] = $job['zip'];
...
if (isset($_GET['mtr'])) {
	...
	//$query = "SELECT u.*, b.storeBrand, COUNT(s.id) as storeCount, s.brandId from users u LEFT JOIN sms_stores as s on s.userId = u.id LEFT JOIN sms_brand as b on b.id = s.brandId WHERE u.accountId ={$account} and u.status =1 GROUP BY u.id ORDER BY u.last_name ASC";
	$query = "SELECT x . *, DATE_FORMAT(x.subscribeDate,'%m/%d/%y') as subDate, c.first_name, c.last_name, c.zip, c.entered, c.mobile, j.title, a.brandId2, ce.city, ce.state_code, g.groupName as groupName, m.type as msgType
	FROM  `candidateXref` x
	LEFT JOIN  `candidate` AS c ON x.candidateId = c.id
	LEFT OUTER JOIN `clickTrack` as t on t.candidateId = c.id and t.brand = x.brandOrig
	LEFT OUTER JOIN `job` as j on j.id = t.jobId
	...
	LEFT OUTER JOIN `group` as g on g.id=(select cg.groupId from `candidate_group` cg where cg.candidateId=x.candidateId and cg.accountId={$accountId} order by cg.groupdate desc limit 0,1)
	LEFT OUTER JOIN `sms_messages` as m on m.id=(select sm.id from `sms_messages` sm where sm.candidateId=x.candidateId and sm.accountId={$accountId} and sm.userId >0 order by sm.msgDate desc limit 0,1)
	WHERE x.brandOrig in (SELECT `brandId` from `account_brand` where `accountId`={$accountId}) and x.accountId = {$accountId} 
	GROUP BY x.candidateId
	ORDER BY x.subscribeDate DESC
	LIMIT 100";	
	...
	foreach ($dbData as $job) {
		...
		//$name = "<a href=\"#updateTraffic\" id=\"updateTraffic\" data-id=\"\'".$job['candidateId']."\'\" data-toggle=\"modal\">."$job['title']"."$job['first_name']." ".$job['last_name']."</a>";
		//$name = "<a href=\"javascript:;\" onclick=\"tj.statusInactiveAll(" . $job['id'] . ");\">Active</a>";
		
		if ($job['groupName']){
		$name = '<a data-toggle="modal" data-id="'.$job['id'].'" class="open-updateCand" href="#updateCand">'.$job['title'].' '.$job['first_name'].' '.$job['last_name'].'('.$job['groupName'].')</a>';	
		}else{
			if ($job['first_name']){
			$name = '<a data-toggle="modal" data-id="'.$job['id'].'" class="open-updateCand" href="#updateCand">'.$job['title'].' '.$job['first_name'].' '.$job['last_name'].'</a>';	
			}else{
			$name = '<a data-toggle="modal" data-id="'.$job['id'].'" class="open-updateCand" href="#updateCand">(Send Text)</a>';	
			}
		}
		
		if (intval($job['msgType'])==3){
		$msg = '<a href="http://www.jobalarm.com/messenger/messageApp/messages.html" target="_blank">(New Msg)</a>   <a href="tel:'.$job['mobile'].'">'.$job['mobile'].'</a>';	
		}else{
		$msg = '<a href="tel:'.$job['mobile'].'">'.$job['mobile'].'</a>';
		}
		
		if ($job['zip']){
			$location = $job['city'].", ".$job['state_code']." ".$job['zip'];
		}else{
			$location = '';
		}
...
if (isset($_GET['bg'])) {
	...
	//$query = "SELECT u.*, b.storeBrand, COUNT(s.id) as storeCount, s.brandId from users u LEFT JOIN sms_stores as s on s.userId = u.id LEFT JOIN sms_brand as b on b.id = s.brandId WHERE u.accountId ={$account} and u.status =1 GROUP BY u.id ORDER BY u.last_name ASC";
	$query = "select s.*, c.longitude as longitude, c.latitude as latitude from sms_stores s left join cities_extended as c on c.zip = s.zip where brandId=9 and active=2";
	$dbData = Config::get('db') -> get_results($query);
	...
	foreach ($dbData as $job) {
		...
		$banners = Config::get('db')->get_results("SELECT s.*, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist FROM `sms_stores` s LEFT JOIN cities_extended ce on ce.zip = s.zip where ((3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958))))<=10) and s.brandId=9 GROUP BY s.storeNum ORDER BY dist ASC");
	
	foreach ($banners as $ban) {
		$jobArrayTemp = array();
						
		$jobArrayTemp[] = $ban['storeNum'];
	
		$jobArrayTemp[] = stripslashes($ban['address']);
		$jobArrayTemp[] = stripslashes($ban['city']);
		$jobArrayTemp[] = $ban['st'];

		$jobArrayTemp[] = $ban['zip'];
		$jobArrayTemp[] = $store.",".$ban['dist'];
...
if (isset($_GET['ux'])) {
	...
	$query = "SELECT s.*, b.storeBrand, s.userId as user, u.first_name, u.last_name FROM sms_stores s LEFT JOIN sms_brand as b on b.id = s.brandId LEFT OUTER JOIN assign_store as a on a.storeId = s.id LEFT OUTER JOIN users as u on u.id = a.userId WHERE a.userId ={$userId} ORDER BY s.st, s.city";
