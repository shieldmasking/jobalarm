...
if (isset($_REQUEST['submitting_candidate'])) {
	...
	$jobs = isset($_REQUEST['jobs']) ? $_REQUEST['jobs'] : '';
...
if ($brand == 6 && $mobile && $promos>0) {
	...
	Config::get('db')->insert('candidateXref',$insert_xref);*/
	
	Config::get('db')->query("insert into candidateXref (candidateId,accountId,promo,job_type,zip,keyword,brandId) values({$candidateId},{$account},{$promos},'{$job_type}','{$zip_code}','{$keyword}',{$brand}) on duplicate key update promo={$promos}, job_type='{$job_type}',zip='{$zip_code}'");
}
}
if (!$mobile && $email) {
...
$keywords = (isset($_REQUEST['job_type'])) ? $_REQUEST['job_type'] : '';
...
if ($account != $ja  && strlen(trim($username)) > 0) {
$query = "
...
<!-- BEGIN HEADER -->
<form action="job.php" id="jaform" name="jaform" method="post">
								
								<input type="hidden" name="submitting_candidate" value="1" />
                           
								
									<h3 class="form-section" style="text-align: center"><strong>Your Local Jobs Are Listed Below!</strong></h3>
									<p class="auto-style3">  
									<strong>Welcome to JobAlarm.com, the only 
									site that wants to help you find a job AND 
									save money at the same time!</strong></p>
									<p>This Employer would like to get more info from you to be able to match jobs to your skills and provide you great deals for their products or services. </p>
...
<option value="Other">Other</option></select>
									...
									<i class="fa fa-briefcase"></i>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea class="form-control" style="height:80px" id="resume_paste" name="resume_paste" placeholder="Add any industry preferences such as Clothing, Sporting Goods, Pets, Fast Food, etc.  Leave blank for all."></textarea>
                                        </div>
                                        <input type="hidden" id="username" name="username" value="Jobalarm" />
									 <input type="hidden" id="accountId" name="accountId" value="212" />
									 <input type="hidden" id="brand" name="brand" value="6" />
									 <input type="hidden" id="smskey" name="smskey" value="JOBALARM" />
								  ...
								  <!--<div class="form-group">
									...
									<strong> Please Send 
										Me Job Related Text Messages.</strong></div>-->

								<div class="form-group"><strong>
								  <input type="checkbox" name="promos" value="1"> 
								  Please 
									Send Me Text Messages with Discounts and Promotions.</strong></div>
