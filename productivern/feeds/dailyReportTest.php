...
$dbMaster = Config::get('db') -> get_results("select a.*, (SELECT COUNT(`id`) FROM `productiveEmailDist` where `accountId`=a.id AND `deptId`=0 AND `sendEmail`=1) as emailCount  FROM `productiveAccount`a where a.id=22 GROUP BY a.id");
//$dbMaster = Config::get('db') -> get_results("select a.*, (SELECT COUNT(`id`) FROM `productiveEmailDist` where `accountId`=8 AND `deptId`=0 AND `sendEmail`=1) as emailCount  FROM `productiveAccount`a where `id`=8 GROUP BY `id`");
...
$addresses = Config::get('db') -> get_results("select * from `productiveEmailDist` where `accountId`={$accountId} AND `deptId`=0 AND `sendEmail`=1 AND `emailAddress` LIKE '%@%' GROUP BY `emailAddress`");
...
.'<p>House Logins: '. $data2[0]['houseCount'] .'</p>'
.'</br>'
