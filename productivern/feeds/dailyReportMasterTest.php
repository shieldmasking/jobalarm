...
//$dbMaster = Config::get('db') -> get_results("select a.*, (SELECT COUNT(`id`) FROM `productiveEmailDist` where `accountId`=a.id AND `deptId`=0 AND `sendEmail`=1) as emailCount  FROM `productiveAccount`a where `emailTime` !='00:00:00' AND TIME_FORMAT(`emailTime` - INTERVAL 1 HOUR, '%H') = TIME_FORMAT(CURRENT_TIME(), '%H') GROUP BY `id`");
$dbMaster = Config::get('db') -> get_results("select a.*, (SELECT COUNT(`id`) FROM `productiveEmailDist` where `accountId`=22 AND `deptId`=0 AND `sendEmail`=1) as emailCount  FROM `productiveAccount`a where `id`=22 GROUP BY `id`");
...
foreach($data2 as $row2){
	...
	*/
	...
	$res1 = floatval($row2['skill1val']);
	}
	
	if(intval($row2['skill2'])==0){
	$res2 ='';
	}else{
	$res2 = floatval($row2['skill2val']);
	}
	
	if(intval($row2['skill3'])==0){
	$res3 ='';
	}else{
	$res3 = floatval($row2['skill3val']);
	}
	
	if(intval($row2['skill4'])==0){
	$res4 ='';
	}else{
	$res4 = floatval($row2['skill4val']);
	}
	
	if(intval($row2['skill5'])==0){
	$res5 ='';
	}else{
	$res5 = floatval($row2['skill5val']);
	}
	
	if(intval($row2['skill6'])==0){
	$res6 ='';
	}else{
	$res6 = floatval($row2['skill6val']);
	}
...
}else if(intval($row2['prodMeasure'])!=2 && intval($row2['prodMeasure'])!=3 && (intval($row2['showEpic'])==1 || intval($row2['showEpicNurse'])==1)){
...
$headerdata2 = Config::get('db') -> get_results("select DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL a.newStartPay DAY),'%m/%d/%y') as startDay, DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL a.newEndPay DAY),'%m/%d/%y') as endDay, DATE_FORMAT(CURDATE(),'%m/%d/%y') AS reportDate, (SUM(CASE WHEN n.userId>0 AND n.dayDate != CURDATE() THEN 1 ELSE 0 END) / SUM(CASE WHEN n.userId>=0 AND n.dayDate != CURDATE() THEN 1 ELSE 0 END) * 100) as reporting, SUM(CASE WHEN n.userId>0 THEN n.avariance ELSE 0 END) / SUM(CASE WHEN n.userId>0 THEN 1 ELSE 0 END) as varAvg, SUM(CASE WHEN n.userId>0 THEN n.gvariance ELSE 0 END) / SUM(CASE WHEN n.userId>0 THEN 1 ELSE 0 END) as varGrid, SUM(CASE WHEN n.userId>0 and n.atotal>0 and n.shift=(SELECT `shift` FROM `productiveAccountXref` where `dept`=n.deptId ORDER BY `shift` DESC LIMIT 0,1) THEN n.atotal ELSE 0 END) / SUM(CASE WHEN n.userId>0 and n.atotal>0 and n.shift=(SELECT `shift` FROM `productiveAccountXref` where `dept`=n.deptId ORDER BY `shift` DESC LIMIT 0,1) THEN 1 ELSE 0 END) as censusShift, SUM(CASE WHEN n.userId>0 THEN n.atotal ELSE 0 END) / SUM(CASE WHEN n.userId>0 THEN 1 ELSE 0 END) as censusAvg, d.dept as deptName, d.prodMeasure FROM `productiveNewData` n left join `ProductiveDept` as d on d.id = n.deptId left join `productiveAccount` as a on a.id=n.accountId where d.active=1 AND n.accountId={$accountId} AND n.dayDate >= DATE_SUB(CURDATE(),INTERVAL a.newStartPay DAY) AND n.dayDate <= DATE_SUB(CURDATE(),INTERVAL a.newEndPay DAY) group by n.accountId, n.deptId ORDER BY d.dept ASC");
...
foreach($headerdata2 as $hdata2){
	if(intval($hdata2['prodMeasure'])!=0 && intval($hdata2['prodMeasure'])!=2 && intval($hdata2['prodMeasure'])!=3){
	$headerAdd .= '<br>' . $hdata2['deptName'] . '  ( Reporting:  ' . ROUND(floatval($hdata2['reporting']),1) . '%   |   Midnight Census:  ' . ROUND(floatval($hdata2['censusShift'] ?? ''),1) . '   |   Reported Census:  ' . ROUND(floatval($hdata2['censusAvg'] ?? ''),1) .'   |   RN Variance:  ' . ROUND(floatval($hdata2['varAvg'] ?? ''),2) . '   |   Grid Variance:  ' . ROUND(floatval($hdata2['varGrid'] ?? ''),2) . ' )';	
	}else{
	//$headerAdd = '<br>' . $hdata2['deptName'] . '  (Reporting:  ' . ROUND(floatval($hdata2['reporting']),1) . '  Variance:  ' . ROUND(floatval($hdata2['varAvg']),2) . ' )';	
	$headerAdd .= '';
	}
}
if(intval($data2[0]['houseLogins'])==1){
$mail->Body    = '<p><a href="https://www.productivern.com/' . $login . '">LOGIN HERE</a></p>'
.'<p>' . $data2[0]['accountName'] . ' Daily Compliance Reporting ('. $data2[0]['reportdate'] .')</p>'
.'<p>House Logins: '. $data2[0]['houseCount'] . '</p><br>PAY PERIOD AVERAGES (IN-PATIENT NURSING)' . $headerAdd
.'<img src="cid:'. substr($data2[0]['accountName'],0,3).'Image">';
}else{
$mail->Body    = '<p><a href="https://www.productivern.com/' . $login . '">LOGIN HERE</a></p>'
.'<p>' . $data2[0]['accountName'] . ' Daily Compliance Reporting ('. $data2[0]['reportdate'] .')</p><br>PAY PERIOD AVERAGES' . $headerAdd
.'<img src="cid:'. substr($data2[0]['accountName'],0,3).'Image">';
}
