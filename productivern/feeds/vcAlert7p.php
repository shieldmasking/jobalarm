...
$dbBrands = Config::get('db') -> get_results("SELECT n.*, d.active, d.prodMeasure, d.dept, s.shift as shiftname, DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -2 HOUR), '%Y-%m-%d') as todayDate FROM `productiveNewData` n left join `ProductiveDept` as d on d.id=n.deptId left join `productiveShifts` as s on s.id=n.shift where d.active=1 and d.prodMeasure !=2 and n.shift=14 and d.accountId=20 and n.userId=0 and n.dayDate= DATE_FORMAT(DATE_ADD(NOW(), INTERVAL -2 HOUR), '%Y-%m-%d') group by n.deptId");
$output = ""; 
if($dbBrands){
	$brandName = '';
	$brandName .='';
