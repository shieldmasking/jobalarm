...
$newdata = Config::get('db') -> get_results("SELECT d.*, a.name as accountName, a.displayProd, DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 2 DAY),'%m/%d/%y') as reportDate from `ProductiveDept` d LEFT JOIN `productiveAccount` as a on a.id=d.accountId WHERE d.active=1 and d.id={$dept} GROUP BY d.id");
...
if(intval($newdata[0]['prodMeasure'])!=2){
...
$w = array(33, 27, 24, 10, $w5, $w6, $w7, $w8, $w9, $w10, $w11, 12, $w15, 55);
}else{
$header2 = array('Unit', 'Date/Shift', 'User', ''. $newdata[0]['uosDesc'] .'', ''. $newdata[0]['skilldesc1'] .'', ''. $newdata[0]['skilldesc2'] .'', ''. $newdata[0]['skilldesc3'] .'', ''. $newdata[0]['skilldesc4'] .'', ''. $newdata[0]['skilldesc5'] .'', ''. $newdata[0]['skilldesc6'] .'','' . $otherDesc . '', 'Variance', 'Notes');
$w = array(33, 27, 24, 18, $x4, $x5, $x6, $x7, $x8, $x9, $x10, 14, 50);
}
...
$dName = "Staffing Detail: " . $newdata[0]['dept'];
	...
	for($i=0;$i<count($header2);$i++){
		$x=$this->GetX();
        $y=$this->GetY();
		if(strlen($header2[$i])>5){
		$this->SetFont('Arial','B',8);	
		}else{
		$this->SetFont('Arial','B',8);	
		}
        $this->Rect($x,$y,$w[$i],14);
		$this->MultiCell($w[$i],7,$header2[$i],0,'C');
		$this->SetXY($x+$w[$i],$y);
...
$deptCount = Config::get('db') -> get_results("select n.deptId, d.dept as deptName  FROM `productiveNewData` n left join `ProductiveDept` as d on d.id = n.deptId left join `productiveAccount` as a on a.id=n.accountId where d.active=1 and d.accountId=8 group by n.accountId, n.deptId");
...
$newdata = Config::get('db') -> get_results("select a.name as accountName, a.displayProd, DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 2 DAY),'%m/%d/%y') AS reportDate FROM `productiveAccount` a where a.id=8 group by a.id");
...
var $widths2;
var $aligns2 = array('L','C','L','C','C','C','C','C','C','C','C','C','L');
...
if(intval($newdata[0]['prodMeasure'])!=2 && intval($newdata[0]['showEpic'])==1){
...
}else if(intval($newdata[0]['prodMeasure'])!=2 && intval($newdata[0]['showEpic'])==0){
...
$deptName = "Tracking Detail: " . $newdata[0]['dept'];
    ...
    for($i=0;$i<count($header3);$i++){
		$x=$this->GetX();
        $y=$this->GetY();
		if(strlen($header3[$i])>9){
		$this->SetFont('Arial','B',8);	
		}else{
		$this->SetFont('Arial','B',8);	
		}
        $this->Rect($x,$y,$w[$i],14);
		$this->MultiCell($w[$i],7,$header3[$i],0,'C');
		$this->SetXY($x+$w[$i],$y);
        //$this->Cell($w[$i],7,$header3[$i],1,0,'C');
	}
    $this->Ln();
...
//$pdf->AddPage4('P');
//$pdf->ImprovedTable($data4);
$getDept = Config::get('db') -> get_results("SELECT n.deptId, n.dayDate, d.active, d.dept, d.prodMeasure1, a.name as accountName, a.displayProd, DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 2 DAY),'%m/%d/%y') as reportDate from `productiveNewData` n LEFT JOIN `productiveAccount` as a on a.id=n.accountId LEFT JOIN `ProductiveDept` as d on d.id=n.deptId WHERE n.dayDate=DATE_FORMAT(DATE_SUB(CURDATE(),INTERVAL 2 DAY),'%Y-%m-%d') and d.active=1 and n.accountId={$accountId} GROUP BY n.deptId ORDER BY d.prodMeasure1 ASC, d.dept ASC");
...
//if(intval($newdata[0]['prodMeasure'])!=2){
//$header2 = array('Unit', 'Date/Shift', 'User', 'Pts', 'Chg', ''. $newdata[0]['nurseDesc'] .'', ''. $newdata[0]['nurse1Desc'] .'', ''. $newdata[0]['nurse2Desc'] .'', ''. $newdata[0]['techLabel'] .'', ''. $newdata[0]['secLabel'] .'', 'Other', 'RN Var', 'Grid Var', 'Notes');
//$w = array(35, 30, 24, 10, $w5, $w6, $w7, $w8, $w9, $w10, $w11, 15, 18, 55);
//}else{
...
//$w = array(35, 30, 24, $x4, $x5, $x6, $x7, $x8, $x9, $x10, 17, 55);
//}

if(intval($data2[0]['prodMeasure'])==2){
$pdf->SetWidths(array(33, 27, 24, 18, $x4, $x5, $x6, $x7, $x8, $x9, $x10, 14, 50));
}else{
$pdf->SetWidths(array(33, 27, 24, 10, $w5, $w6, $w7, $w8, $w9, $w10, $w11, 12, $w15, 55));	
}
...
foreach($data2 as $row2){
	...
	if(intval($row2['useGrid'])>=1 && intval($row2['useGrid'])<=4){
	$gvariance = round(floatval($row2['gvariance']),1);
	}else if(intval($data2[0]['prodMeasure'])==2 && (intval($row2['useGrid'])==0 || intval($row2['useGrid'])==5)){
	$gvariance = round(floatval($row2['hrsVariance']),1);
	}else{
	$gvariance = '';	
	}
	
	$uos = round(floatval($row2['procedureCount']),0);	
	$rnVar = '-';
...
$pdf->Row(array($row2['deptName'],$date,$submittedby,$uos,$res1,$res2,$res3,$res4,$res5,$res6,$otherval,$gvariance,$note));	
}
...
}else if(intval($row2['prodMeasure'])!=2 && intval($row2['showEpic'])==1){
...
foreach($data2 as $row2){
	...
	$census = '';
			...
			switch ($row2['oneto1Acuity']) {
				case "1":
					$oneto1 = $oneto1 + floatval($row2['oneto1']);
					break;
				case "2":
					$oneto2 = $oneto2 + floatval($row2['oneto1']);
					break;
				case "3":
					$oneto3 = $oneto3 + floatval($row2['oneto1']);
					break;
				case "4":
					$oneto4 = $oneto4 + floatval($row2['oneto1']);
					break;
				case "5":
					$oneto5 = $oneto5 + floatval($row2['oneto1']);
					break;
				case "6":
					$oneto6 = $oneto6 + floatval($row2['oneto1']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto1']);
					break;
				default:
					$oneto1 = 0.00;
				}
				
			switch ($row2['oneto2Acuity']) {
				case "1":
					$oneto1 = $oneto1 + floatval($row2['oneto2']);
					break;
				case "2":
					$oneto2 = $oneto2 + floatval($row2['oneto2']);
					break;
				case "3":
					$oneto3 = $oneto3 + floatval($row2['oneto2']);
					break;
				case "4":
					$oneto4 = $oneto4 + floatval($row2['oneto2']);
					break;
				case "5":
					$oneto5 = $oneto5 + floatval($row2['oneto2']);
					break;
				case "6":
					$oneto6 = $oneto6 + floatval($row2['oneto2']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto2']);
					break;
				default:
					$oneto2 = 0.00;
				}
				
			switch (round(floatval($row2['oneto3Acuity']),1)) {
				case "1.0":
					$oneto1 = $oneto1 + floatval($row2['oneto3']);
					break;
				case "2.0":
					$oneto2 = $oneto2 + floatval($row2['oneto3']);
					break;
				case "3.0":
					$oneto3 = $oneto3 + floatval($row2['oneto3']);
					break;
				case "3.5":
					$oneto3 = $oneto3 + floatval($row2['oneto3']);
					break;
				case "4.0":
					$oneto4 = $oneto4 + floatval($row2['oneto3']);
					break;
				case "5.0":
					$oneto5 = $oneto5 + floatval($row2['oneto3']);
					break;
				case "6.0":
					$oneto6 = $oneto6 + floatval($row2['oneto3']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto3']);
					break;
				default:
					$oneto3 = 0.00;
				}
			
			switch (round(floatval($row2['oneto4Acuity']),1)) {
				case "1.0":
					$oneto1 = $oneto1 + floatval($row2['oneto4']);
					break;
				case "2.0":
					$oneto2 = $oneto2 + floatval($row2['oneto4']);
					break;
				case "3.0":
					$oneto3 = $oneto3 + floatval($row2['oneto4']);
					break;
				case "4.0":
					$oneto4 = $oneto4 + floatval($row2['oneto4']);
					break;
				case "5.0":
					$oneto5 = $oneto5 + floatval($row2['oneto4']);
					break;
				case "6.0":
					$oneto6 = $oneto6 + floatval($row2['oneto4']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto4']);
					break;
				default:
					$oneto4 = 0.00;
				}
				
			switch (round(floatval($row2['oneto5Acuity']),1)) {
				case "1.0":
					$oneto1 = $oneto1 + floatval($row2['oneto5']);
					break;
				case "2.0":
					$oneto2 = $oneto2 + floatval($row2['oneto5']);
					break;
				case "3.0":
					$oneto3 = $oneto3 + floatval($row2['oneto5']);
					break;
				case "4.0":
					$oneto4 = $oneto4 + floatval($row2['oneto5']);
					break;
				case "5.0":
					$oneto5 = $oneto5 + floatval($row2['oneto5']);
					break;
				case "6.0":
					$oneto6 = $oneto6 + floatval($row2['oneto5']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto5']);
					break;
				default:
					$oneto5 = 0.00;
				}
				
			switch (round(floatval($row2['oneto6Acuity']),1)) {
				case "1.0":
					$oneto1 = $oneto1 + floatval($row2['oneto6']);
					break;
				case "2.0":
					$oneto2 = $oneto2 + floatval($row2['oneto6']);
					break;
				case "3.0":
					$oneto3 = $oneto3 + floatval($row2['oneto6']);
					break;
				case "4.0":
					$oneto4 = $oneto4 + floatval($row2['oneto6']);
					break;
				case "5.0":
					$oneto5 = $oneto5 + floatval($row2['oneto6']);
					break;
				case "6.0":
					$oneto6 = $oneto6 + floatval($row2['oneto6']);
					break;
				case "0.5":
					$twoto1 = $twoto1 + floatval($row2['oneto6']);
					break;
				default:
					$oneto6 = 0.00;
				}
...
$mailAcct=8;
$addresses = Config::get('db') -> get_results("select * from `productiveEmailDist` where `accountId`={$mailAcct} AND `deptId`=0 AND `sendEmail`=1 AND `emailAddress` LIKE '%@%' GROUP BY `emailAddress`");
...
$mail->FromName = 'PERFECTSHIFT Report';

//foreach ($addresses as $address) {
//    $mail->AddAddress(trim($address['emailAddress']));
//}

//$mail->AddAddress('joe.hardman@tuality.org');  // Add a recipient
//$mail->AddAddress('Colby.Russell@tuality.org');  // Add a recipient
//$mail->AddAddress('meredith.peterson@tuality.org');  // Add a recipient
//$mail->AddAddress('Amanda.Elegant@tuality.org');  // Add a recipient
//$mail->AddAddress('carol.holm@tuality.org');  // Add a recipient
//$mail->AddAddress('connie.lively@tuality.org');  // Add a recipient
//$mail->AddAddress('cyndi.christy@tuality.org');  // Add a recipient
//$mail->AddAddress('cynthia.trif@tuality.org');  // Add a recipient
//$mail->AddAddress('heather.way@tuality.org');  // Add a recipient
//$mail->AddAddress('jan.fitch@tuality.org ');  // Add a recipient
//$mail->AddAddress('jen.mccollum@tuality.org');  // Add a recipient
//$mail->AddAddress('jen.packer@tuality.org');  // Add a recipient

//$mail->AddAddress('john.mccann@tuality.org ');  // Add a recipient
//$mail->AddAddress('Kaylyn.Jones@tuality.org');  // Add a recipient
//$mail->AddAddress('kim.lierman@tuality.org');  // Add a recipient
//$mail->AddAddress('lindsey.kennedy@tuality.org');  // Add a recipient
//$mail->AddAddress('Lisa.Fischer@tuality.org');  // Add a recipient
//$mail->AddAddress('Lisa.Middleton@tuality.org');  // Add a recipient
//$mail->AddAddress('lynne.padua@tuality.org');  // Add a recipient
//$mail->AddAddress('maria.vargas@tuality.org');  // Add a recipient
//$mail->AddAddress('mary.lucaskoach@tuality.org');  // Add a recipient
//$mail->AddAddress('matthew.kivela@tuality.org');  // Add a recipient
//$mail->AddAddress('mike.hurtado@tuality.org');  // Add a recipient
//$mail->AddAddress('julia.seifert@tuality.org');  // Add a recipient
//$mail->AddAddress('martha.ragan@tuality.org');  // Add a recipient
//$mail->AddAddress('paula.fleskes@tuality.org');  // Add a recipient
//$mail->AddAddress('Rae.Isable@tuality.org');  // Add a recipient
//$mail->AddAddress('sarika.singh@tuality.org');  // Add a recipient
//$mail->AddAddress('Stefanie.Briscoe@tuality.org');  // Add a recipient
//$mail->AddAddress('Dan.Schlegel@tuality.org');  // Add a recipient
//$mail->AddAddress('evamarie.mace@tuality.org');  // Add a recipient
//$mail->AddAddress('Alix.Ulrich@tuality.org');  // Add a recipient

//$mail->AddAddress('susan.costello@perfectshift.com');  // Add a recipient
//$mail->AddBCC('candy.goulette@perfectshift.com');  // Add a recipient
//$mail->AddBCC('ron.hughes@perfectshift.com');  // Add a recipient
//$mail->AddBCC('kevin.spellman@perfectshift.com');  // Add a recipient
//$mail->AddBCC('john.kalb@perfectshift.com');  // Add a recipient
//$mail->AddBCC('erik.elisson@perfectshift.com');  // Add a recipient
...
$mail->AddEmbeddedImage('/home/prnadmin/public_html/dailyreports/vcImage'.$data2[0]['filedate'].'.jpg', 'vcImage');
$mail->IsHTML(true);                                  // Set email format to HTML
$mail->Subject = 'Daily Compliance Reporting Details: HMC ('. $data2[0]['reportdate'] . ')';
$mail->Body    = '<p>HMC Daily Compliance Reporting ('. $data2[0]['reportdate'] .')</p>'
.'</br>'
.'<img src="cid:vcImage">';
