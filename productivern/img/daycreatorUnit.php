...
function createDays(){	
$dbBrands = Config::get('db') -> get_results("select a.*, d.active, d.prodMeasure, d.crashLog, d.hppd, d.daysWk, d.charge, DAYOFWEEK(curdate()) as weekDay from `productiveAccountXref` a LEFT JOIN `ProductiveDept` as d on d.id = a.dept where d.active>0 and a.dept=431 AND d.prodMeasure !=2");
