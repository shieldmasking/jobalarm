...
function GenerateReports() {
    ...
    ");	
		...
		if(($canData[0]['actualCount'] + $canData[0]['planCount'])>0){
...
function GetAllUsers() {
	...
	if(intval($role)>1){
        ...
        LEFT JOIN `productiveUserRoles` as r on r.id=u.role
        LEFT JOIN `ProductiveDept` as d on d.id=x.deptId
        WHERE u.active>0 GROUP BY u.id ORDER BY u.last_name,u.first_name ASC
		";
	}
    ...
    if($dbData){
		...
		if($nUser==0){
			...
			if(intval($role)>7){
				...
				if($escalationId >=1 && $aproductivity =='' ){
				$style = "1";
...
function UpdateProdTest() {
	...
	$nurseAdd = 0;
	$requirednurses = 0;
	If(intval($acuityTotal)==1){
	$patienttotal = intval($oneto1)+intval($oneto2)+intval($oneto6)+intval($high)+intval($med)+intval($low);
	}else{
	$patienttotal = (isset($_REQUEST['patienttotal'])) ? $_REQUEST['patienttotal'] : 0;
	}
