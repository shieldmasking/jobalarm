...
$label = intval($_SESSION['account']['labelId']);
$enterpriseId = intval($_SESSION['account']['enterpriseId']);
            ...
            <div class="m-portlet m-portlet--mobile">
                <div class="m-portlet__body">
				 <div class="row">
						...
						<?php if($role >6): ?>
						<div class="col-sm-6">
							<div class="pull-right" <?php echo $director; ?>>
								<div class="order-xs-1 col-sm-6 col-md-6 col-lg-6 col-xl-6" style="text-align:right">
						
                                        <button type="button" class="btn btn-success" data-target="#addnewUnits" data-toggle="modal" <?php echo $addUnit; ?>>
                                            Add Unit
                                        </button>                         
					 </div>
                                <!--end: Dropdown-->
                    ...
                    <div class="row">
                        ...
                        <table id="UnitsTable"  class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%" style="color:black">
                            <thead>
                                <tr>
    <!--                                <th></th>-->
    <!--                                <th></th>-->
                                    <th width="20%" data-priority="1">Unit</th>
									<th width="20%" data-priority="1">Service Line</th>
                                    <th width="10%" data-priority="2">Unit ID</th>
									 <th width="15%" data-priority="3">Unit Measure</th>
                                    <th width="15%" data-priority="4">Unit of Service</th>
									<!--<th width="15%" data-priority="3">Prod% (30 Day Avg.)</th> -->
									<th width="20%" data-priority="5">Manager</th>
                                </tr>
                            </thead>
                        </table>
						</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="add_unit" class="m-form m-form--fit m-form--label-align-right">
                        <div class="m-portlet__body">
						<input id="userRole" type="hidden" value="<?php echo $role; ?>" />
							<div class="form-group" <?php echo $hidden; ?>>
                               	<label for="accountSelect" class="col-form-label">Account*:
								</label>
                                <div class="col-8">
                                    <select id="accountSelect" type="select" class="form-control">
									<option value="0">Select</option>
								<?php
								if($role<100){
								$dbDir = Config::get('db')->get_results("SELECT * from `productiveAccount` WHERE `label`={$label} order by `name` ASC");
								}else{
								$dbDir = Config::get('db')->get_results("SELECT * from `productiveAccount` order by `name` ASC");	
								}
							...
							<div class="form-group">
                                ...
                                <div class="col-8">
                                    <select id="newDirector" type="select" class="form-control">
									<option value="0">Select Director</option>
															
								<?php
								if($role<90 && $role>12){
								$dbData = Config::get('db')->get_results("SELECT *, CONCAT(`last_name`, ', ', `first_name`) as newName from `productiveUser` where `accountId`={$accountId} and `role`>=6 and `role`<9 and u.last_name !='' group by `id` order by `last_name`, `first_name` ASC");
								}else if($role>89 && $role<100){
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where a.label={$label} and u.role>=6 and u.role<9 and u.active>0 and u.last_name !='' group by u.id order by u.last_name ASC, u.first_name ASC");	
								}else{
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where u.accountId={$accountId} AND u.role>=6 and u.role<9 and u.active>0 and u.last_name !='' group by u.id order by u.last_name ASC, u.first_name ASC");	
								}
								
								foreach($dbData as $b) 
								{ 
								?>
								<option value="<?php echo $b['id'];?>"><?php echo $b['newName'];?> </option>
								<?php 
								} 
								?>
							</select>
							...
							<div class="form-group">
                                ...
                                <div class="col-8">
                                    <select id="newManager" type="select" class="form-control">
									<option value="0">Select Manager</option>
															
								<?php
								if($role<90 && $role>12){
								$dbData = Config::get('db')->get_results("SELECT *, CONCAT(`last_name`, ', ', `first_name`) as newName from `productiveUser` where `accountId`={$accountId} and `role`>=6 and `role`<8 and u.last_name !='' order by `last_name`, `first_name` ASC");
								}else if($role>89 && $role<100){
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name,' - ',r.role, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where a.label={$label} and u.role>=6 and u.active>0 and u.last_name !='' and u.role<8 order by u.last_name ASC, u.first_name ASC");	
								}else{
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name,' - ',r.role, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where u.accountId={$accountId} AND u.role>=6 and u.active>0 and u.last_name !='' and u.role<8 order by u.last_name ASC, u.first_name ASC");	
								}
							...
							<div class="form-group">
								...
								<select id="unitprodMeasure" type="select" class="form-control">
										...
										<option value=4>HPPV (Visits)</option>
										<option value=6>HPPDel (Deliveries)</option>
...
<div class="m-content" id="unitDetails">
    <!--Begin::Main Portlet-->
            ...
            <div class="m-portlet m-portlet--mobile">
                <div class="m-portlet__body bg-light">
				 ...
				 <div class="row">
							...
							<div class="col-6">
                                ...
                                <div class="col-8">
                                    <input class="form-control m-input" type="text" value="" id="editunitNameMgr" <?php echo $director; ?>>
                                </div>
							...
							<div class="col-6">
                                ...
                                <div class="col-4">
                                    <input class="form-control m-input" type="text" value="" id="editunitNumberMgr" <?php echo $director; ?>>
                                </div>
                            </div>
							</div>
							</div>
							...
							<div class="col-6">
                                ...
                                <div class="col-8">
                                    <select id="unitManagerMgr" type="select" class="form-control" <?php echo $manager; ?>>
									<option value="0">Select New Manager</option>
									<option value="-1">No Manager (Remove this Manager)</option>
															
								<?php
								if($role<90 && $role>12){
								$dbData = Config::get('db')->get_results("SELECT *, CONCAT(`last_name`, ', ', `first_name`) as newName from `productiveUser` where `accountId`={$accountId} and `role`>=6 and u.last_name !='' and `role`<8 order by `last_name`, `first_name` ASC");
								}else if($role>89 && $role<100){
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name,' - ',r.role, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where a.label={$label} and u.role>=6 and u.active>0 and u.role<8 and u.last_name !='' order by u.last_name ASC, u.first_name ASC");	
								}else{
								$dbData = Config::get('db')->get_results("SELECT u.*, CONCAT(u.last_name, ', ', u.first_name, ' (', a.name,' - ',r.role, ')') as newName from `productiveUser` u LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT JOIN `productiveUserRoles` as r on r.id=u.role where u.accountId={$accountId} AND u.role>=6 and u.active>0 and u.role<8 and u.last_name !='' order by u.last_name ASC, u.first_name ASC");	
								}
								foreach($dbData as $m) 
								{ 
								?>
								<option value="<?php echo $m['id'];?>"><?php echo $m['newName'];?> </option>
								<?php 
								} 
								?>
										</select>
							...
							<div class="col-6">
                                ...
                                <div class="col-8">
                                    <select id="unitDirector" type="select" class="form-control" <?php echo $director; ?>>
									<option value="0">Select New Director</option>
									<option value="-1">No Director (Remove this Manager)</option>
															
								<?php
								if($role<90 && $role>12){
								$dbDir = Config::get('db')->get_results("SELECT *, CONCAT(`last_name`, ', ', `first_name`) as newName from `productiveUser` where `accountId`={$accountId} and `role`>=6 and `role`<9 and u.last_name !='' group by `id` order by `last_name`, `first_name` ASC");
								}else if($role>89 && $role<100){
								...
								{ 
								?>
								<option value="<?php echo $d['id'];?>"><?php echo $d['newName'];?> </option>
								<?php 
								} 
								?>
										</select>
							...
							<button type="button" class="btn btn-info btn-sm" data-target="#addCategory" data-toggle="modal">
								...
								<?php
								if($role>12 && $role<90){
								$dbCat = Config::get('db')->get_results("SELECT c.*, a.name, CONCAT(a.name, ' - ', c.categoryName) as catName from `productiveCategory` c LEFT JOIN `productiveAccount` as a on a.id=c.accountId where a.enterpriseId={$enterpriseId} OR c.accountId=0 order by a.accountId DESC, a.name ASC, c.categoryName ASC");
								}else if($role>89 && $role<100){
								$dbCat = Config::get('db')->get_results("SELECT c.*, a.name, CONCAT(a.name, ' - ', c.categoryName) as catName from `productiveCategory` c LEFT JOIN `productiveAccount` as a on a.id=c.accountId where a.label={$label} order by a.name ASC, c.categoryName ASC");	
								}else{
								$dbCat = Config::get('db')->get_results("SELECT c.*, a.name, CONCAT(a.name, ' - ', c.categoryName) as catName from `productiveCategory` c LEFT JOIN `productiveAccount` as a on a.id=c.accountId where c.accountId={$accountId} OR c.accountId=0 order by a.name ASC, c.categoryName ASC");	
								}
								foreach($dbCat as $c) 
								{ 
								?>
							...
							<div class="form-group" id="budget" hidden>
							<div class="row">
							<div class="col-md-6">
							...
							<div class="col-md-6">
							    <label for="budgetValue" class="col-form-label" hidden>Daily Budget Value($):</label>
								<div class="col-4">
                                <input type="number" class="form-control number" id="budgetValue" style="text-align: right" hidden>
							</div>
							</div>
							</div>
							</div>
						 ...
						 <div class="row">
						 <div class="col-md-6" id="nursing3">
						...
						<div class="col-md-6" id="nurse2">
						 ...
						 <div class="col-md-6">
								...
								<div class="col-4">
                                <select type="select" class="form-control" id="churn" <?php echo $disabled; ?>>
								<option value="0">No</option>
								<option value="1">Yes</option>
								</select>
							</div>
				 ...
				 <div class="form-group">
							...
							<div class="col-6">
                                <label for="editunitprodMeasureMgr" class="col-form-label">
                                    Productivity/UOS Measure:
                                </label>
                                <div class="col-6">
								<select id="editunitprodMeasureMgr" type="select" class="form-control" <?php echo $disabled; ?>>
										<option value=1>HPPD</option>
										<option value=2>WHPUOS</option>
										<option value=4>HPPV (Visits)</option>
										<option value=6>HPPDel (Deliveries)</option>
							...
							<div class="col-6">
							    <label for="editunitprodValueMgr" class="col-form-label">UOS Value:</label>
								<div class="col-4">
                                <input type="number" class="form-control number" id="editunitprodValueMgr" style="text-align: right" <?php echo $disabled; ?>>
							</div>
                            </div>
							</div>
							</div>
						<div class="form-group" id="nursing2">
							<div class="row">
							<div class="col-md-6">
                               	<label for="editcensusShiftMgr" class="col-form-label">Census Time:</label>
                                <div class="col-6">
                                    <select id="editcensusShiftMgr" type="select" class="form-control" <?php echo $disabled; ?>>
										<option value=52>12:00 Midnight</option>
										<option value=50>1:00 AM</option>
										<option value=48>2:00 AM</option>
										<option value=46>3:00 AM</option>
							...
							<div class="row">
							<div class="col-6">
                                <label for="inshiftProd" class="col-form-label" >
                                    Calculate In-Shift Productivity Estimate:
                                </label>
                                <div class="col-8">
								<select id="inshiftProd" type="select" class="form-control" <?php echo $disabled; ?>>
										<option value="0">No</option>
										<option value="1">Yes - Census at beginning of day</option>
										<option value="2">Yes - Census at end of day</option>
										</select>
                                    
                                </div>
							</div>
							<div class="col-6">
                                <label for="rnThreshold" class="col-form-label">
                                    RN Variance Threshold (+/-):
                                </label>
                                <div class="col-4">
								<select id="rnThreshold" type="select" class="form-control" >
							...
							<div class="row">
							<div class="col-md-6">
							<label for="thresholdHigh" class="col-form-label">High Indicator Value % (ie. 105.0):</label>
								<div class="col-4">
                                <input type="number" class="form-control number" id="thresholdHigh" style="text-align: right" <?php echo $disabled; ?>>
							</div>
							</div>
							<div class="col-md-6">
							<label for="thresholdLow" class="col-form-label">Low Indicator Value %: (ie. 95.0)</label>
								<div class="col-4">
                                <input type="number" class="form-control number" id="thresholdLow" style="text-align: right" <?php echo $disabled; ?>>
							</div>
                            </div>
							</div>
							</div>
				...
				<hr></hr>
				<div class="form-group" id="nursing4">
				<div class="title" style="padding-top: 20px;">
				 <h4>Acuity Levels</h4>
				 </div>
				 <div class="title">
				 <p>Select the Acuity Levels appropriate for this Unit.  Descriptions can be as simple as High, Med, Low or more specific for each Acuity Level.</p>
				 </div>
				...
				<div id="skill">
				 ...
				 <div class="title">
				 <p>Optional:  Define up to 5 Resources for this Support/Ancillary Unit.  Default is one field labeled "Hours".</p>
				 </div>
						...
						<div class="form-group">
							...
							<div class="col-md-6">
							<label for="skillbudget1" class="col-form-label" hidden>Hourly Budget Value($):
							
							<input type="number" id="skillbudget1" class="form-control number" hidden>
							</label>
							</div>
						</div>
						</div>
						...
						<div class="form-group">
							...
							<input type="number" id="skillbudget2" class="form-control number" hidden>
							</label>
							</div>
						</div>
						</div>
						<div class="form-group">
							...
							<input type="number" id="skillbudget3" class="form-control number" hidden>
							</label>
							</div>
						</div>
						</div>
						<div class="form-group">
							...
							<input type="number" id="skillbudget4" class="form-control number" hidden>
							</label>
							</div>
						</div>
						</div>
						<div class="form-group">
							...
							<input type="number" id="skillbudget5" class="form-control number" hidden>
							</label>
							</div>
						</div>
						</div>
				  ...
				  <div class="title"><p>These are resources typically not in staffing but their hours apply to your productivity (ie. Director, Manager, Clinician, etc.).</p>
				  </div>
				  <div>
				...
				<hr></hr>
				 ...
				 <div class="title">
				 <p>These are resources that require temporary access to this Unit (ie. Charge Nurse coming over from another unit or a Manager backing you up for vacation).</p>
				 </div>
				  <div>
                ...
                <div class="modal-body">
                    <form role="form">
                        ...
                        <div class="form-body">
							...
							<div class="form-group form-md-line-input">
                                <label for="grantText"><p>Allow User to receive Text Alerts for this Unit?</p></label>
                ...
                <div class="modal-body">
                    <form role="form">
                        ...
                        <div class="form-body">
							...
							<input type="number" id="resourcevalue" class="form-control number" hidden>
