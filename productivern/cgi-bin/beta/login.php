...
//$p = (isset($_POST['page'])) ? $_POST['page'] : 0;
        if (intval($p) > 0) {
            ...
            $header = 'location: view.php?u=' . $p2 . '';
        } else {
            $header = 'location: dashboard.php';
        }

		$query = "SELECT u.*, r.label, UNIX_TIMESTAMP() as login_time, e.image as enterpriselogo, IFNULL(d.prodMeasure,1) as prodMeasure, IFNULL(a.payPeriod,0) as payPeriod, DAYOFYEAR(curdate()) as currentDay, DAYOFYEAR(a.payPeriodFirst) as firstDay, COUNT(DISTINCT d.prodMeasure) as prodDistinct, a.payPeriodFirst as startPay, IFNULL(a.image,'') as image, count(IFNULL(d.prodMeasure,1)) as prodCount, a.type, a.status, a.name as company, IFNULL(l.login,'') as labelLogin, IFNULL(l.id,'') as labelId, IFNULL(l.logo,'') as logo, IFNULL(l.favicon,'') as favicon, IFNULL(l.labelName,'') as labelName, IFNULL(l.supportEmail,'') as supportEmail, IFNULL(l.supportLink,'') as supportLink, IFNULL(l.videoLink,'') as videoLink, u.deptId as deptid, d.dept as deptName, d.unitId, a.active as acctActive FROM `productiveUser` u LEFT OUTER JOIN `productiveDeptXref` as x on x.userId = u.id LEFT JOIN `productiveUserRoles` as r on r.id=u.role LEFT OUTER JOIN `ProductiveDept` as d on d.id = x.deptId LEFT JOIN `productiveAccount` as a on a.id = u.accountId LEFT OUTER JOIN `productiveEnterprise` as e on e.id = a.enterpriseId LEFT JOIN `productiveLabel` as l on l.id = a.label WHERE (u.email='{$email}' OR u.userName='{$email}') AND u.pwd='{$password}' GROUP BY u.id";
        ...
        if (count($dbData) > 0) {
            ...
            }else if ($_SESSION['account']['type'] == 1) {
                header('location: ob/dashboard.php');
            }else {
                header($header);
            }
            exit();
...
<head>
<meta charset="utf-8"/>
<title>ProductiveRN Login</title>
...
<div class="page-header">
        ...
        <div class="page-header-menu" style="background-color: #282A3C">
            <div class="container">
                ...
                <!-- BEGIN TOP BAR MENU -->
                <div class="col-md-6 col-sm-6">
                    <ul class="list pull-right" style="margin-top:15px">
                        
                        <li><a href="/contact.php" style="font-size:14px;color:white">Contact Us</a></li>
