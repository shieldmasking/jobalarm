...
if ($acctAdd){
$dbMobile = Config::get('db')->get_results("SELECT s.id as brandOrig, s.storeBrand, s.twitterDemo, s.color, s.keyword, s.positions, s.type, s.storeImage, s.deal, a.logo, a.billing_plan, a.brandId as industry, a.logopic, a.miles, a.status as accType, ad.adImg, ad.active as adActive, ad.adLink, ad.id as adLinkId from `sms_brand` s left outer join `account` as a on a.id ={$acctAdd} left outer join `ads` as ad on ad.brandId = s.id where s.id ={$brandOrig} group by s.id");
...
$groupBy2 = "GROUP BY j.brand, j.city, j.address, j.title";
//$searchAdd2 =" AND j.postId !={$acctAdd} AND j.jobCats !={$industry}";
$searchAdd2 =" AND j.postId !={$acctAdd} AND j.jobCats !={$industry}";	
