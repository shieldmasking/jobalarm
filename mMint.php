...
if (intval($acctAdd)>0 && (intval($accType)==2 || intval($accType)>5)){
...
$limit2 = " LIMIT 0,15";
}else if(intval($acctAdd)>0 && intval($accType)==5){
$groupBy = "GROUP BY j.brand, j.priority, j.zipCode, j.address, j.title";
$groupBy2 = "GROUP BY j.brand, j.title";
$searchAdd2 =" AND j.priority != 1 AND j.postId !={$acctAdd} AND j.jobCats !={$industry}";	
$brandAdd = " AND j.postId={$acctAdd}";
$priority = " AND j.priority!=3";
$limit2 = " LIMIT 0,15";
}else if(intval($accType)==0){
$groupBy = "GROUP BY j.brand, j.zipCode, j.title";
$groupBy2 = "GROUP BY j.brand, j.zipCode, j.title";	
$searchAdd2 = " AND j.brand !={$brandOrig}";
$brandAdd = " AND j.brand={$brandOrig}";
$accType = 5;
$limit2 = " LIMIT 0,10";
}else if(intval($accType)==4){
$groupBy = "GROUP BY j.brand, j.zipCode, j.title";
$groupBy2 = "GROUP BY j.brand, j.zipCode, j.title";	
$searchAdd2 = " AND j.brand ={$brandOrig} AND j.jobCats !={$industry}";
$brandAdd = " AND j.brand={$brandOrig}";
//$accType = 5;
$limit2 = " LIMIT 0,10";
}else{
...
if (intval($brandOrig)==276 || intval($brandOrig)==7){
$brandAdd = " AND (j.brand=276 OR j.brand=7)";
$hideAdd = 'hidden';
}
...
$dbData = Config::get('db')->get_results("SELECT SQL_CALC_FOUND_ROWS j.*, j.id as jobId, a.status as acctStatus, s.userPic as userPic, s.storeBrand, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist 
	...
	where 
        (3963*ACOS(SIN(({$lat}+.000001)/57.2958)*SIN(ce.latitude/57.2958)+ COS(({$lat}+.000001)/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-(({$lon}+.000001)/57.2958))))<=$zipDist and j.status>0 and j.postId>0$brandAdd$searchAdd
	$groupBy 
	order by priority ASC, dist asc, j.brand asc
	Limit 0,50");
}else{
$dbData = Config::get('db')->get_results("SELECT SQL_CALC_FOUND_ROWS j.*, j.id as jobId, a.status as acctStatus, a.brandId as industry, s.userPic as userPic, s.storeBrand, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist 
	FROM job j
	    ...
	    inner join cities_extended ce on ce.zip = j.zipCode
	where 
        (3963*ACOS(SIN(({$lat}+.000001)/57.2958)*SIN(ce.latitude/57.2958)+ COS(({$lat}+.000001)/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-(({$lon}+.000001)/57.2958))))<=$zipDist and j.status>0$priority$searchAdd2$searchAdd 
		$groupBy2 
		order by priority ASC, dist ASC
	$limit2");
	
	if(count($dbData)>0 && intval($accType)<=6 && intval($acctAdd)>0){
		$limit = "LIMIT 0,15";
...
$featuredData = Config::get('db')->get_results("SELECT SQL_CALC_FOUND_ROWS j.*, j.id as jobId, j.postId as featured, a.status as acctStatus, s.userPic as userPic, s.storeBrand, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist 
	...
	$groupBy
	order by j.priority ASC, dist ASC
	$limit
	");
...
<!--
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2545585330917467"
        ...
        $(document).on('change', 'input', function () {
            ...
            for (var i = 0; i < options.length++; i++) {   
                if (options[i].value == $(this).val()) {
                    //alert($(this).val());
					window.location.replace('http://www.jobalarm.biz/m.php?a=' + acctAdd + '&m=' + mobile + '&b=' + brandOrig + '&z=' + zipcode + '&t=' + status + '&lon=' + longi + '&lat=' + lati + '');
                    break;
                }
            }
        })
              ...
              <div class="m-portlet__body">
				<div class="row">
						  <div class="col-md-12">
                           ...
                           <h4 class="form-section" style="text-align: center"><strong><?php echo $msg; ?></strong></h4></br>
...
<div id="googlead" class="text-center">											
<a href="https://mint-mobile.58dp.net/c/298234/1156941/7915" target="_top" id="1156941"><img src="//a.impactradius-go.com/display-ad/7915-1156941" border="0" alt="" width="300" height="250"/></a><img height="0" width="0" src="https://mint-mobile.58dp.net/i/298234/1156941/7915" style="position:absolute;visibility:hidden;" border="0" />
</div>	
...
<div class="col-lg-6">
<input type="text" id="city" class="form-control m-input" list="zips" value="<?php echo $zipcode; ?>" />
<datalist id="zips">
<?php
foreach($cityDb as $f) { 
?>
...
foreach($jobsList as $tweet) {
    ...
    if ($featuring && !isset($tweet['featured'])) {
		...
		echo '<a href="https://mint-mobile.58dp.net/c/298234/1374732/7915" target="_top" id="1374732"><img src="//a.impactradius-go.com/display-ad/7915-1374732" border="0" alt="" width="300" height="250" align="center"/></a><img height="0" width="0" src="https://mint-mobile.58dp.net/i/298234/1374732/7915" style="position:absolute;visibility:hidden;" border="0" />';
    } 
    $jobsCounter++;
    $id = $tweet['jobId'];
	...
	if ($searchAdd){
		...
		$moreLike = '<a href="http://www.jobalarm.biz/m.php?z='.$zipcode.'&a='.$acctAdd.'&m='.$candidateId.'&b='.$brandOrig.'&s='.$moreSearch.'&p='.$morePriority.'">More jobs like this one</a>';
	}
...
<!-- JAMobile -->
<a href="https://mint-mobile.58dp.net/c/298234/1374737/7915" target="_top" id="1374737"><img src="//a.impactradius-go.com/display-ad/7915-1374737" border="0" alt="" width="320" height="50" align="center"/></a><img height="0" width="0" src="https://mint-mobile.58dp.net/i/298234/1374737/7915" style="position:absolute;visibility:hidden;" border="0" />
