...
if (isset($_REQUEST['restart'])) {
    chdir('../twitapp');
    //echo getcwd();
	$restartstatus = shell_exec('forever stop 0');
   // $restartstatus = shell_exec('forever start -o twitapp.log -e twiterr.log -m 10000 --minUptime=1 --spinSleepTime=1 app.js');
    $restartstatus = shell_exec('forever start app.js');
    echo $restartstatus;
    //header('location: sitestatus.php');
}
...
<body class="page-boxed page-header-fixed page-container-bg-solid page-sidebar-closed-hide-logo page-sidebar-closed">
    ...
    <div class="page-header">
        ...
        <div class="page-header-menu" style="background-color:#444d58">
            <div class="container">
                <h1 style="width:340px;float:left;margin:0;padding:5px;color:white">System Status</h1>
                <!-- BEGIN MEGA MENU -->
                        ...
                        <div class="portlet light ">
                            <div class="portlet-title">
                                <div class="caption caption-md">
                                    ...
                                    <span class="caption-subject theme-font bold uppercase">Information</span>
                                </div>
                            </div>
                            <div class="portlet-body">
                            <span style="font-size:16px;color:black">Twitter Stream Status: </span>
                            <?php
                                $status = shell_exec('ps x | grep app.js');
                                if (strpos($status,'/usr/bin/node /home/tweetedjobs/twitapp/app.js')) {
                                    echo '<span style="color:green;font-weight:bold;font-size:18px;">RUNNING</span>';
                                } else {
                                    echo '<span style="color:red;font-weight:bold;font-size:18px;">DOWN</span> <a style="font-size:18px;" href="sitestatus.php?restart=1">Restart</a>';
                                }
                            ?>
                            <br /><br />
                            <span style="font-size:16px;color:black">Tweets in Process Queue: </span>
                            <?php
                                $count = shell_exec("mongo twitter --quiet --eval 'print(db.jobtweets.find().count())'");
                                echo '<span style="color:blue;font-weight:bold;font-size:18px;">'.$count.'</span>';
                            ?>
                            </div>
                        </div>
                        <!-- END PORTLET-->
...
<![endif]-->
    ...
    <script src="inc/tweetedjobs-admin-status.js" type="text/javascript"></script>
<!--     <script>
