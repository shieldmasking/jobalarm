...
//                }
    ...
    public static function getSMSGridData($options = NULL) {
        $orderBy = " sms_history.messageDate DESC ";
        $userId = Config::get('loggedIn');
        ...
        if (isset($_REQUEST['search'])) {
            ...
            foreach ($searchFields as $field => $value) {
                ...
                if ($field == 'filter_zipCode' && !isset($searchFields['filter_zipdist'])) {
                    ...
                    $whereArray[] = " zipCode IN {$zipCodes} ";
                } else if ($field == 'filter_optout') {
                    $value--;
                    if ($value >= 0) {
                        $whereArray[] = " (people.optOut = {$value})";
                    }
                } else if ($field == 'filter_mobileNum') {
                    $whereArray[] = " (sms_history.mobileNum LIKE '{$value}%')";
                } else if ($field == 'filter_surveyid') {
                    $whereArray[] = " (sms_history.surveyId = {$value})";
                } else if ($field == 'filter_keyword') {
                    $responseAppend .= " (" . Utility::buildKeywordQuery('searchData', $value) . " OR " . Utility::buildKeywordQuery('resumeSearch', $value) . ")";
                } else if ($field == 'filter_stageid' || $field == 'filter_eventid') {
                    $filter = str_replace('[]', '', $field);
                    $filtervar = substr($filter, 7);
                    //var_dump($filtervar);
                    $decodevalue = $value;
                    ...
                    if ($filtervar == 'stageid') {
                        if (is_array($decodevalue)) {
                            $whereArray[] = "(responsestage.stageId IN ('" . implode("','", $decodevalue) . "'))";
                        } else {
                            if ($decodevalue > 0) {
                                $whereArray[] = "(responsestage.stageId = '{$decodevalue}')";
                    ...
                    if ($filtervar == 'eventid') {
                        if (is_array($decodevalue)) {
                            $whereArray[] = "(responseevent.eventId IN ('" . implode("','", $decodevalue) . "'))";
                        } else {
                            if ($decodevalue > 0) {
                                $whereArray[] = "(responseevent.eventId = '{$decodevalue}')";
                            }
                        }
                    }
                    ...
                    if (is_array($value)) {
                        ...
                        if ($value != "-1") {
                            $whereArray[] = substr($field, 7) . " LIKE '" . $value . "%'";
                        }
                    }
                }
            }
            ...
            if (count($searchArray) > 0) {
                $havingAdd = "AND " . implode(' AND ', $searchArray);
            }
            if (count($whereArray) > 0) {
                $whereAdd = "AND " . implode(' AND ', $whereArray);

                if (!$searchRemoved)
                    $whereAdd .= ' AND (responsestage.stageId <> 999 or responsestage.stageId is null) ';
            }
        } else {
            if (!$searchRemoved)
                $whereAdd .= ' AND (responsestage.stageId <> 999 or responsestage.stageId is null) ';
        ...
        sms_history.id,
        sms_history.message,
        sms_history.viewed as smsviewed,
        sms_history.mobileNum,
        sms_history.messageDate,
        sms_history.messageDate as updated,
	...
	LEFT join response
	on response.surveyId=sms_history.surveyId AND response.mobileNum=sms_history.mobileNum

	LEFT JOIN people
	on people.mobileNum = response.mobileNum
        ...
        INNER JOIN (
            select mobileNum,MAX(messageDate) as maxdate FROM sms_history where type=1 and isReply=1 group by mobileNum
        ) smsh ON sms_history.mobileNum = smsh.mobileNum AND messageDate = maxdate

 	WHERE sms_history.type = 1 AND sms_history.isReply = 1  AND ((sms_history.isReply=1 AND sms_history.userId={$userId}) or (sms_history.type=2 AND sms_history.userId={$userId})) {$sqlAdd}

        {$responseSubQuery}
        {$whereAdd1} {$whereAdd} {$accessWhere}
        ...
        foreach ($dbData as $record) {
            $viewed = ($record['smsviewed'] == 1) ? '' : 'font-weight:bold';
            $statusImage = '';
            $isOptOut = 0;
            ...
            $outputarray['records'][] = array(
                ...
                'recid' => $record['id'],
                'status' => $statusImage,
                'stage' => (isset($stageLookup[$record['stage']])) ? $stageLookup[$record['stage']] : '',
                'event' => (isset($eventLookup[$record['event']])) ? $eventLookup[$record['event']] : '',
                'responseId' => $record['surveyResponseId'],
                ...
                'updated' => $record['messageDate'],
                'viewed' => $record['smsviewed'],
                'style' => $style,
