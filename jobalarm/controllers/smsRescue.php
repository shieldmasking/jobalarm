                  ...
                  //if ((strtoupper(substr($keyword,0,3)) == 'CVS' && in_array(strtoupper($rcvdmsg),$demo)) || (!$isreply && !$iscmd)) {   
						...
						if ($xrefdata && count($xrefdata) > 0) {
							...
							//}
								...
								$outmessage = "Please reply with your name, location, your situation and if it is life threatening.  DO NOT GO INTO YOUR ATTIC!";
								}else if (intval($brandType) ==6){
								$outmessage = "Please reply with your name and rescue capabilities, ie. boat, 4WD, etc. ";
								}else{
							...
							if (intval($brandType) <=2){
								...
								$outmessage = "Please reply with your name, SPECIFIC location, your situation and if it is life threatening.  DO NOT GO INTO YOUR ATTIC!";
								}else if (intval($brandType) ==6){
								$outmessage = "Please reply with your name and rescue capabilities, ie. boat, 4WD, etc. ";
								}else{
								$outmessage = "{$thisbrand['storeBrand']} Job Alerts. Submit your application @ {$acctLink}. Reply HELP for help or STOP to cancel. Msg&Data Rates May Apply";	
                        ...
                        $xrefdata = Config::get('db')->get_results("select x.*, c.accountXref, c.zip, c.stageId as stage, b.type as brandType, b.website from `candidateXref` as x left join `candidate` as c on x.candidateId = c.id left join `job` as j on j.brand = x.brandOrig left join `clickTrack` as l on l.mobile = c.mobile and l.trackId = j.twitterId left join `sms_brand` as b on b.id = x.brandOrig where x.brandOrig={$brandId} and x.candidateId={$candidateId} group by l.trackId order by x.subscribeDate DESC");
						$acct = $xrefdata[0]['accountId'];
						...
						//} else if (strlen(trim($rcvdmsg)) > 4) {
							...
							if (intval($promo) > 0 && intval($promo) < 3) {
								...
								if (intval($brandOrig) == 188 || intval($brandOrig)== 187){
								$stageId = $stageId + 2;
								}else{
								$stageId = $stageId + 1;
								}
								...
								$outmessage = "We received your message and will do our best to coordinate help. If we get confirmed help, we will text you back.";
								}else if (intval($stageId) ==2 && intval($brandId) == 188){
								$outmessage = "Thank you!  We will send you help requests. When you receive a request, please confirm that you will respond.";
								}else{
