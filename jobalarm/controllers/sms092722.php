...
//        $keyword = $vals[2]['value'];
    ...
    public static function receive2($keyword = '') {
        ...
        if ($user){
            ...
            $promo = 1;
            $candidatedata = Config::get('db')->get_results("select * from `candidate` where `mobile`='{$mobileNum}' or `mobile`='{$emobile}'");
            $candidateId = 0;
            if ($candidatedata && count($candidatedata) > 0 && !$lastKeyword) {
                ...
                Config::get('db')->insert('candidate', $insertdata);
				$newCandidate = Config::get('db')->get_results("select * from candidate where mobile='{$mobileNum}'");
            ...
            if ($branddata && count($branddata) > 0) {
                    ...
                    if (!$isreply && !$iscmd) {
                        ...
                        if ($xrefdata && count($xrefdata) > 0) {
                            ...
                            } else if (intval($brandType) == 20) {
                                $outmessage = "Welcome to {$xrefdata[0]['storeBrand']}'s Perinatal Concierge, start your amazing journey here: {$periLink}&m={$mobileNum}.  Msg&Data Rates May Apply.";
                            } else if (intval($brandType) == 100) {
                                $outmessage = "Welcome to Productivity Alerts, Message and Data Rates May Apply. Reply HELP for help or STOP to cancel.";
                        ...
                        if (intval($aliasType) == 2) {
                            ...
                            if (intval($bType) == 4) {
                                ...
                                $jobLink = "http://jobalarm.biz/m.php?z=" . $zip . "&m=" . $mobileNum . "&b=" . $brandOrig . "" . $linkAdd . ". Mobile Apply Now!";
                            }
