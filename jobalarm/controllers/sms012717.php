...
class Sms {
    ...
    public static function getAllCandidate($candidateId) {
        ...
        $query = "SELECT 
            ...
            AND brandId IN(select `brandId` from `assign_brand` where userId={$userx})            
            AND type <2
	...
	public static function receive($keyword='') {
		...
		If (strtoupper(substr($keyword,0,5)) == 'ARBYS' && strtoupper($rcvdmsg) == '') {
		...
		If (strtoupper(substr($keyword,0,8)) == 'BLIZZARD' && strtoupper($rcvdmsg) == '') {
