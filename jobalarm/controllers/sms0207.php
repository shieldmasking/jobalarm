...
//        $keyword = $vals[2]['value'];
  //      print_r($keyword);
		...
		$keywords = Config::get('db')->get_results("SELECT s.*, a.keyword as alias, a.accountId as aAcct FROM `sms_brand` s left outer join `sms_brand_alias` as a on a.brandId = s.id ORDER BY s.keyword");
			...
			$keyword3 = Config::get('db')->get_results("SELECT s.*, a.keyword as alias FROM `sms_brand` s left join `sms_brand_alias` as a on a.brandId = s.id where a.keyword ='".strtoupper({$keyword2})."'");
			$keyData = $keyword3[0];
			$keyword2 = $keyData['keyword'];
			$keyAlias = $keyData['alias'];
			$search = substr($keyAlias, strpos($keyAlias, " ") + 1);
				if (strtoupper(substr($search,0,5)) == 'DEALS') {
					$rcvdmsg = 'DEALS';
					$search = '';
				}else{
				$rcvdmsg = '';
        ...
        if ($candidatedata && count($candidatedata) > 0) {
            ...
            if ($candidateId > 0) {
                ...
                //}else{
				//$branddata = Config::get('db')->get_results("SELECT c.*, x.brandOrig as brandId, s.type, s.keyword, s.deal, s.storeBrand FROM `candidate` c left outer join `candidateXref` as x on x.candidateId = c.id left outer join `sms_brand` as s on s.id = x.brandOrig WHERE c.mobile ='{$mobileNum}' order by x.subscribeDate desc");	
				//}
			if ($branddata && count($branddata) > 0) {
                    $thisbrand = $branddata[0];
				...
				if (!$isreply && !$iscmd) {
                  //if ((strtoupper(substr($keyword,0,3)) == 'CVS' && in_array(strtoupper($rcvdmsg),$demo)) || (!$isreply && !$iscmd)) {   
						...
						$xrefdata = Config::get('db')->get_results("select * from `candidateXref` where brandOrig={$brandOrig} and candidateId={$candidateId}");	
                        ...
                        $xrefData = Config::get('db')->get_results("select x.*, c.accountXref from `candidateXref` as x left join `candidate` as c on x.candidateId = c.id where x.brandId={$brandOrig} and x.candidateId={$candidateId}");
						
						if(count($xrefData)>0){
						$acct = $xrefData[0]['accountId'];
						$keySearch = $xrefData[0]['search'];
						...
						if (strtoupper(substr($rcvdmsg,0,5)) == 'DEALS') {
							...
							$outmessage = "Welcome to {$thisbrand['storeBrand']} Deals. Here's your first deal:{$thisbrand['deal']}. Msg&Data Rates May Apply. Reply STOP to Cancel or HELP for Support.";
                            $debug = self::send_sms_message2($outmessage,$user,$keyword2,$brandId,$candidateId,$acct);
								$type = 8;
								$updatedata = array(
								'brandId'=>$brandId,
