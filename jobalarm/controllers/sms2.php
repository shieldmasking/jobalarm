...
class Sms {
	...
	//public static function validate_zipcode($rcvdmsg) {
    //return preg_match('/^[0-9]{5}([- ]?[0-9]{4})?$/', $rcvdmsg);
	//}
        ...
        if (strlen($user) == 11 && strlen($keyword) > 0) {
            ...
            if ($candidateId > 0) {
                ...
                if ($branddata && count($branddata) > 0) {
                    ...
                    if (!$isreply && !$iscmd) {
                        //var_dump($thisbrand);
                        $xrefdata = Config::get('db')->get_results("select * from candidateXref where brandId='{$thisbrand['id']}' and candidateId={$candidateId}");
                        ...
                        if (strtoupper(substr($rcvdmsg,0,5)) == 'DEALS') {
                            ...
                            $debug = self::send_sms_message($outmessage,$user,$keyword);                            
						} else if (strtoupper(substr($rcvdmsg,0,5)) == 'TODAY') {
                            ...
                            if ($xrefdata && count($xrefdata) > 0) {
                                $outmessage = "Your {$thisbrand['storeBrand']} jobs: http://jobalarm.com/m.php?z={$rcvdmsg}&m={$mobileNum}&b={$thisbrand['id']} Also, if you would like DEALS & DISCOUNT msgs from {$thisbrand['storeBrand']}, reply DEALS";
                                $debug = self::send_sms_message($outmessage,$user,$keyword);                            
                            }
						} else {		
								$updatedata = array('candidateId'=>intval($candidateId));
								$updatedata = array('message'=>($rcvdmsg));
								Config::get('db')->insert('sms_messages',$updatedata);
								$outmessage = "not a zip";
								$debug = self::send_sms_message($outmessage,$user,$keyword);
									if (strtoupper($userId) != strtoupper($candidatedata[0]['accountXref'])){
									$updatedata = array('accountXref'=>intval($userId));
									$updatewhere = array('mobile'=>$mobileNum);
									Config::get('db')->update('candidate',$updatedata,$updatewhere);
									}
							}    
