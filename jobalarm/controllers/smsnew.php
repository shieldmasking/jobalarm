...
class Sms {
	...
	public static function send() {
        if (isset($_REQUEST['recips']) && count($_REQUEST['recips']) > 0) {
            ...
            foreach ($recips as $candidateId) {
				...
				$query = "select c.*, x.keyword, x.keyword2, x.brandId as brandX, x.brandOrig, x.userXref as userId, x.promo, cg.groupId as cgGroup from candidate c LEFT OUTER JOIN `candidate_group` as cg on c.id = cg.candidateId and cg.accountId={$userId} LEFT JOIN candidateXref as x on x.candidateId = c.id LEFT JOIN account_brand as a on a.brandId=x.brandId where c.id ={$candidateId} and (x.brandId = a.brandId or x.brandId=6 or x.brandId=19) group by c.mobile";
				...
				$keyword2 = (isset($dbData[0]['keyword2'])) ? $dbData[0]['keyword2'] : 0;
				...
				'userId'=>Config::get('db')->filter($userx),
    ...
    public static function send_sms_message($message,$mobile,$keyword,$brandId,$candidateId) {
        $header = 'Content-Type: application/xml';
        $output = "";
	...
	public static function receive($keyword='') {
		...
		$promo = 0;
		$brandOrig = 0;
		$demo = array("BOJANGLES","CHURCHS","DONUTS","TOOLS","FISH","MURPHY","PAPA","PETCO","POPEYES","ADVANCE","BLIZZARD","SHAKE","SONIC","BOSTON","KING","NAPA","CAPTAIN","WENDYS","WALGREENS","RITE","BELL","SPEEDWAY","GAP","CASTLE","TACO","CHICK","ZAXBYS","HOWIES","MCDONALDS","DOMINOS","HUT","HARDEES","KFC","POLLO","PANERA","WHOPPER","CARLS","WICH");
		If (strtoupper(substr($keyword,0,4)) == 'MALL' && strtoupper($rcvdmsg) == 'JOBS'){
				$isreply = '';
				$outmessage2 = "Somebody just tried the Keyword MALL ";
				$user2 = "12148500163";
				$brandId2 = 23;
				$candidateId2 = 4717;
				$debug2 = self::send_sms_message($outmessage2,$user2,$keyword,$brandId2,$candidateId2);
		}
		If (strtoupper(substr($keyword,0,5)) == 'DEMO2' && in_array(strtoupper($rcvdmsg),$demo)) {
						$keyword2 = strtoupper($rcvdmsg);
		...
		If (strtoupper(substr($keyword,0,5)) == 'DEMO2' && strtoupper($rcvdmsg)=='') {
						$rcvdmsg = "ADVANCE";
						$keyword2 = strtoupper($rcvdmsg);
			            $outmessage2 = $user." just tried JobAlarm for ".$keyword2;
						$user2 = "12148500163";
		...
		If (strtoupper(substr($keyword,0,6)) == 'DOLLAR' && strtoupper($rcvdmsg) == '') {
                        $outmessage2 = $user." just tried JobAlarm for DOLLAR ";
						$user2 = "12148500163";
						$brandId2 = 27;
						$candidateId2 = 4717;
                        $debug2 = self::send_sms_message($outmessage2,$user2,$keyword,$brandId2,$candidateId2);
		}
		If (strtoupper(substr($keyword,0,6)) == 'BLIZZARD' && strtoupper($rcvdmsg) == '') {
		...
		If (strtoupper(substr($keyword,0,6)) == 'WENDYS' && strtoupper($rcvdmsg) == '') {
		...
		If (strtoupper(substr($keyword,0,6)) == 'SONIC' && strtoupper($rcvdmsg) == '') {
                        $outmessage2 = $user." just tried JobAlarm for SONIC ";
						$user2 = "12148500163";
						$brandId2 = 39;
						$candidateId2 = 4717;
                        $debug2 = self::send_sms_message($outmessage2,$user2,$keyword,$brandId2,$candidateId2);
		}
		If (strtoupper(substr($keyword,0,5)) == 'LOVES' && strtoupper($rcvdmsg) == '') {
                        $outmessage2 = $user." just tried JobAlarm for LOVES ";
						$user2 = "12148500163";
						$brandId2 = 13;
						$candidateId2 = 4717;
                        $debug2 = self::send_sms_message($outmessage2,$user2,$keyword,$brandId2,$candidateId2);
        ...
        if ($candidatedata && count($candidatedata) > 0) {
            ...
            if ($candidateId > 0) {
                If (strtoupper(substr($keyword,0,5)) == 'DEMO2') {
					...
					$branddata = Config::get('db')->get_results("select * from sms_brand where keyword='".strtoupper($keyword2)."'");
					}else{
			...
			if ($branddata && count($branddata) > 0) {
                ...
                //if (!$isreply && !$iscmd) {
                  if ((strtoupper(substr($keyword,0,3)) == 'CVS' && strtoupper(substr($rcvdmsg,0,6)) == 'RETAIL') || (!$isreply && !$iscmd)) {   
						$xrefdata = Config::get('db')->get_results("select x.*, c.accountXref from `candidateXref` as x left join `candidate` as c on x.candidateId = c.id where x.brandId={$brandIdOrig} and x.candidateId={$candidateId}");	
						...
						if ($xrefdata && count($xrefdata) > 0) {
							if ($xrefdata[0]['promo']==0 && strtoupper($keyword) == strtoupper($thisbrand['keyword'])){
								...
								$debug = self::send_sms_message($outmessage,$user,$keyword,$brandId,$candidateId);
							} else if ($xrefdata[0]['type']==2){
							$outmessage = "Welcome to {$thisbrand['storeBrand']} Job Alerts! Up to 4 msgs/month. Reply HELP for help or STOP to stop. Msg&Data Rates May Apply";
							$debug = self::send_sms_message($outmessage,$user,$keyword,$brandId,$candidateId);
							} else if ($xrefdata[0]['type']==1){
							$outmessage = "Welcome to {$thisbrand['storeBrand']} Job Alerts! Up to 4 job messages per month. Reply HELP for help or STOP to cancel. Msg&Data Rates May Apply";
							$debug = self::send_sms_message($outmessage,$user,$keyword,$brandId,$candidateId);
							} else if (strtoupper(substr($keyword,0,3)) == 'CVS' && strtoupper(substr($rcvdmsg,0,6)) == 'RETAIL'){
							usleep(30000000);
							$outmessage = "Thanks for your interest in {$thisbrand['storeBrand']} retail jobs. A Recruiter will be in contact soon regarding our Store Manager in Training positions.";
							$debug = self::send_sms_message($outmessage,$user,$keyword,$brandId,$candidateId);
							} else {
							...
							$outmessage = "Welcome to {$thisbrand['storeBrand']} Job Alerts! Reply with your ZIP CODE to receive your local jobs. Reply HELP for help or STOP to cancel. Msg&Data Rates May Apply";
                            $debug = self::send_sms_message($outmessage,$user,$keyword,$thisbrand['id'],$candidateId);
						...
						//} else if (strlen(trim($rcvdmsg)) > 4) {
    						...
    						$updatedata = array('zip'=>intval($zip));
                            $updatewhere = array('mobile'=>$mobileNum);
                            Config::get('db')->update('candidate',$updatedata,$updatewhere);
                            if ($xrefdata && count($xrefdata) > 0) {
                                $outmessage = "Your local {$thisbrand['storeBrand']} jobs are here: http://jobalarm.com/m.php?z={$zip}&m={$mobileNum}&b={$thisbrand['id']}";
								$debug = self::send_sms_message($outmessage,$user,$keyword,$brandId,$candidateId);   
							}
							if (intval($promo) > 0 && intval($promo) < 3) {
								...
								$userXdata = Config::get('db')->get_results("select c.*, x.userXref FROM candidate c LEFT JOIN candidateXref as x on x.candidateId = c.id where c.mobile='{$mobileNum}' and (x.brandId={$brandId} or x.brandId=6 or x.brandId=19)");	
								$userX = $userXdata[0]['userXref'];	
								$updatedata = array(
									'userId'=>intval($userX),
