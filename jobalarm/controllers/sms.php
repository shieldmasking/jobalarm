...
// More headers
    ...
    public function findLocation($candidateId, $brandId, $zip)
    {
        $storenum = '';
        $aType = 0;
        $acct = '';

        $storesDb = Config::get('db')->get_results("select * from `cities_extended` where zip={$zip}");

        if ($storesDb) {
            $lat = $storesDb[0]['latitude'];
            $lon = $storesDb[0]['longitude'];

            $storeData = Config::get('db')->get_results("select s.*, ce.zip, a.status as aType, (3963*ACOS(SIN({$lat}/57.2958)*SIN(ce.latitude/57.2958)+ COS({$lat}/57.2958) *COS(ce.latitude/57.2958)*COS((ce.longitude/57.2958)-({$lon}/57.2958)))) as dist from `sms_stores` s left join `account` as a on a.id = s.accountId left join `cities_extended` as ce on ce.zip = s.zip where s.brandId={$brandId} and ce.zip is not null order by dist ASC LIMIT 0,5");
            if ($storeData) {
                $dist = $storeData[0]['dist'];

                if (intval($dist) < 20 && intval($dist) >= 0) {
                    $storenum = $storeData[0]['id'];
                    $acct = $storeData[0]['accountId'];

                    Config::get('db')->query("update `candidateXref` set `accountId` ={$acct}, `stageId` ={$storenum}  WHERE `candidateId` ={$candidateId} AND `brandOrig` ={$brandId}");
                }
            }
        }
    ...
    public function send_messages_with_optout($smsAlex, $number, $keyword)
    {
        $header = 'Content-Type: application/xml';
        $output = "";
        $post = '';
    ...
    public static function receive($keyword = '')
    {
        ...
        if (strlen($user) == 11) {
            ...
            if ($candidatedata && count($candidatedata) > 0) {
				...
				$mobileId = Config::get('db')->get_results("select * from candidate where mobile='{$mobileNum}'");
				$candidateId = $mobileId[0]['id'];
            }
            $outmessage = '';
            if ($candidateId > 0) {
                ...
                if ($branddata && count($branddata) > 0) {
                    ...
                    if (!$isreply && !$iscmd) {
                        ...
                        if (strtoupper(substr($rcvdmsg, 0, 5)) == 'DEALS') {
                            ...
                            if (intval($bType) == 0) {
                                $accType = self::findLocation($candidateId, $brandId, $zip);
                            }
                            ...
                            if (intval($bType) == 4) {
                                ...
                                $jobLink = "http://jobalarm.biz/m.php?z=" . $zip . "&m=" . $candidateId . "&b=" . $brandId . "" . $linkAdd . ". Mobile Apply Now!";
                            }

                            if ($xrefdata && count($xrefdata) > 0) {
                                //$outmessage = "Thank You!  Here are your local {$thisbrand['storeBrand']} jobs: http://jobalarm.biz/m.php?z={$zip}&m={$mobileNum}&b={$brandId}$linkAdd";
...
//        $keyword = $vals[2]['value'];
    ...
    public function newapply($candidate, $brand)
    {
    ...
    public static function receive2($keyword = '') {
        ...
        if ($keyword2 && in_array(strtoupper($keyword2), $alias)) {
            $keydb = Config::get('db')->get_results("SELECT s.*, a.keyword as alias, a.deal as aliasDeal, a.type as aliasType, a.periLink FROM `sms_brand` s left join `sms_brand_alias` as a on a.brandId = s.id where a.keyword ='" . strtoupper($keyword2) . "'");
            $keyData = $keydb[0];
            $keyword2 = $keyData['keyword'];
			$lastKeyword = $keyData['keyword'];
            $keyAlias = trim($keyData['alias']);
            $aliasType = $keyData['aliasType'];
            $aliasDeal = $keyData['aliasDeal'];
			$periLink = $keyData['periLink'];
			$brandType = $keyData['type'];
        ...
        if ($user){
            $mobileNum = substr($user, 1);
			//$emobile = md5($mobileNum);
			$emobile = openssl_encrypt(filter_var($mobileNum, FILTER_SANITIZE_STRING), $ciphering, $encryption_key, $options, $encryption_iv);
            $promo = 1;
            $candidatedata = Config::get('db')->get_results("select * from `candidate` where `newMobile`='{$emobile}'");
            $candidateId = 0;
            if ($candidatedata && count($candidatedata) > 0 && !$lastKeyword) {
                $candidateId = $candidatedata[0]['id'];
                $candidateZip = $candidatedata[0]['zip'];
				//$keyword2 = $candidatedata[0]['lastKeyword'];
				$branddata = Config::get('db')->get_results("SELECT c.*, x.brandOrig as brandId, s.accountId as acct, s.type as type, s.keyword, s.storeBrand, s.textLimit, s.active as brandActive, s.website FROM `candidate` c left outer join `candidateXref` as x on x.candidateId = c.id left outer join `sms_brand` as s on s.id = x.brandOrig WHERE c.id ={$candidateId} and x.keyword2 = c.lastKeyword order by x.subscribeDate desc");
            }else if ($candidatedata && count($candidatedata) > 0 && $lastKeyword) {
                $updatedata = array('lastKeyword' => $lastKeyword);
				$updatewhere = array('id' => $candidatedata[0]['id']);
                Config::get('db')->update('candidate', $updatedata, $updatewhere);
				$candidateId = $candidatedata[0]['id'];
                $candidateZip = $candidatedata[0]['zip'];
				$branddata = Config::get('db')->get_results("SELECT c.*, s.id as brandId, s.accountId as acct, s.type as type, s.keyword, s.storeBrand, s.textLimit, s.active as brandActive, s.website FROM `candidate` c left outer join `sms_brand` as s on s.keyword ='{$lastKeyword}' WHERE c.id ={$candidateId}");
            }else if (!$candidatedata && $lastKeyword){
                $insertdata = array('newMobile' =>openssl_encrypt(filter_var($mobileNum, FILTER_SANITIZE_STRING), $ciphering, $encryption_key, $options, $encryption_iv),
									'lastKeyword' => $lastKeyword);
                Config::get('db')->insert('candidate', $insertdata);
				$eMobile=openssl_encrypt(filter_var($mobileNum, FILTER_SANITIZE_STRING), $ciphering, $encryption_key, $options, $encryption_iv);
				$newCandidate = Config::get('db')->get_results("select * from candidate where newMobile='{$eMobile}'");
            ...
            if ($branddata && count($branddata) > 0) {
                    ...
                    if (!$isreply && !$iscmd) {
                        ...
                        $xrefdata = Config::get('db')->get_results("select x.*, c.accountXref, b.type as brandType, b.storeBrand, b.website, b.active as brandActive from `candidateXref` as x left join `candidate` as c on x.candidateId = c.id left join `sms_brand` as b on b.id = x.brandOrig where x.brandOrig={$brandOrig} and x.candidateId={$candidateId}");
                        ...
                        if ($xrefdata && count($xrefdata) > 0) {
                            ...
                            if (intval($brandType) <= 4) {
                                $outmessage = "Welcome to {$xrefdata[0]['storeBrand']} Job Alerts! Reply with your ZIP CODE to receive your local jobs. Reply HELP for help or STOP to cancel. Msg&Data Rates May Apply";
                            }else {
								exit();
                                //$outmessage = "{$xrefdata[0]['storeBrand']} Job Alerts. Submit your application @ {$acctLink}. Reply HELP for help or STOP to cancel. Msg&Data Rates May Apply";
                            }
                            ...
                            if (intval($thisbrand['brandActive']) >= 1 && $brandOrig !=178 && $brandOrig !=264 && $brandOrig !=278) {
                                $insertXrefData = array(
                                    'candidateId' => $candidateId,
                                    'accountId' => $acct,
                                    'shortCode' => 58046,
                                ...
                                Config::get('db')->update('candidate', $updatedata, $updatewhere);
                            }else if(intval($thisbrand['brandActive'])== 0 && $brandOrig !=178 && $brandOrig !=264) {
                        ...
                        $xrefdata = Config::get('db')->get_results("select x.*, a.status as accType, c.accountXref, c.zip, c.stageId as stage, b.type as brandType, b.website, l.id as trackId from `candidateXref` as x left outer join `account` as a on a.id = x.accountId left join `candidate` as c on x.candidateId = c.id left join `clickTrack` as l on l.candidateId = c.id and l.accountId = x.accountId left join `sms_brand` as b on b.id = x.brandOrig where x.brandOrig={$brandOrig} and x.candidateId={$candidateId}");
                        $acct = $xrefdata[0]['accountId'];
                        $keySearch = $xrefdata[0]['keyword'];
                        $brandOrig = $xrefdata[0]['brandOrig'];
                        $click = $xrefdata[0]['trackId'];
                        ...
                        if (intval($aliasType) == 2) {
                            ...
                            $msgType = 2;
                            $updatedata = array('zip' => str_pad($zip, 5, '0', STR_PAD_LEFT));
                            $updatewhere = array('newMobile' =>openssl_encrypt(filter_var($mobileNum, FILTER_SANITIZE_STRING), $ciphering, $encryption_key, $options, $encryption_iv));
                            Config::get('db')->update('candidate', $updatedata, $updatewhere);
                            ...
                            if (intval($accType)> 0 && intval($acct)==0 ) {
                                $linkAdd .= "&a=" . $accType;
                            } else {
                                $linkAdd .= "&a=" . $acct;
                            }
                            if (intval($bType) == 4) {
								...
								//$jobLink = "http://jobalarm.biz/m.php?z=" . $zip . "&b=" . $brandOrig . "" . $linkAdd . ". Mobile Apply Now!";
                                $jobLink = "https://jobalarm.biz/m.php?z=" . $zip . "&b=" . $brandOrig . "&m=" . $candidateId . "" . $linkAdd . ". Mobile Apply Now!";
								//$jobLink = "http://jobalarm.biz/m.php?z=" . $zip . "&m=" . $candidateId . "&b=" . $brandId . "" . $linkAdd . ". Mobile Apply Now!";
                            }
                            ...
                            if ($xrefdata && count($xrefdata) > 0 && $brandId == 6 && $brandOrig != 9) {
                                $outmessage = "Thank You! Please go here to complete your profile: https://jobalarm.biz/app/?b=6&a=301&z={$zip}&m={$candidateId}";
                                $debug = self::send_sms_message2($outmessage, $user, $keyword2, $brandId, $candidateId, $acct, $msgType);
                            }
                            /*
                        if (intval($promo) > 0 && intval($promo) < 3) {
...
/*
							...
							*/
							if (intval($stageId) < 2 && intval($brandId) == 187) {
