...
class Candidates {
    ...
    public static function doView() {
        ...
        if (Router::getGetVar('job')) {
            ...
            $dbData = Config::get('db')->get_results('SELECT name from survey where surveyId=' . $surveyId);
            Config::push('jsvars', array('surveyName' => $dbData[0]['name']));
        }
        Config::push('scripts', 'views/shared/sharedfunctions.js');
        Config::push('scripts', 'views/candidates/candidates.js');
        Config::set('mainmenu', true);
        Config::set('sysblock', true);
        require_once('views/candidates/candidates.php');
    }
