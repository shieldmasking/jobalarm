...
tj.loadProd = function() {
	...
	if (!tj.prodLoaded) {
        ...
        jQuery('#staffingView').load('views/staffing.php', {}, function () {
			...
			tj.staffingdaterangepickerInit();
			//tj.initializeUserAssignedGrid('');
            // tj.initializeJobsGrid('');
            // tj.initializeUsersGrid('');
            tj.prodLoaded = true;
...
tj.buildPostpartumChart = function(id){
    var ctx = document.getElementById(id);
    tj.postpartumReport = new Chart(ctx, {
        type: 'line',
        data: {},
        options: {
            scales: {
...
tj.updateReports = function(start,end) {
    ...
    tj.getReportData(start,end,function(data){
        ...
        $('#reportPromoNumber').html(Math.max(data.totalPromo,0));
...
tj.editUser = function(userId) {
        ...
        $.ajax({
            ...
            success:function(response) {
				...
				$('#ecf_mobile').val(response.data.mobile);
        ...
        console.log(userId);
		//console.log('roleId',response.data.roleId);
...
tj.addNewUser = function () {
		...
		var mobile = $('#ecf_newuser_mobile').val();
...
tj.initializeProdGrid = function(id) {
    tj.prodStartDate = moment().format('YYYY-MM-DD');
    tj.prodEndDate = moment().format('YYYY-MM-DD');
...
tj.updateProd = function() {
        ...
        $.ajax({
            ...
            success:function(response) {
				...
				alert('Your Total Patient Count does not add up to match your Patient Selections.  Please correct this.');
				}else if (response.data.message == true) {
				$('#dataId').val(response.data.dataId);
				...
				//document.getElementById("shift_add").selectedIndex = "";
				//document.getElementById("day_add").selectedIndex = "";
				//document.getElementById("dataId").selectedIndex = "";
				//document.getElementById("note").selectedIndex = "";
                $('#addProd').modal('hide');
...
tj.updateNewProd = function (day,shift) {
		...
		var ccs = $('#ccs_add').val();
		var ps1 = $('#psl_add').val();
		var postcount = $('#postcount_add').val();
		var ppvag = $('#ppvag_add').val();
		var ppwc = $('#ppwc_add').val();
		var ppmag = $('#ppmag_add').val();
				
        if (tj.debug) console.log('add new matrix');
        ...
        $.ajax({
            ...
            success: function (result) {
				...
				//tj.userAssignedTable.ajax.reload();
				tj.prodTable.ajax.reload();
				alert('Matrix added successfully.');
				document.getElementById("chargecount_add").selectedIndex = "";
				document.getElementById("techcount_add").selectedIndex = "";
				document.getElementById("seccount_add").selectedIndex = "";
				document.getElementById("antecount_add").selectedIndex = "";
				...
				document.getElementById("ccs_add").selectedIndex = "";
				document.getElementById("ps1_add").selectedIndex = "";
				document.getElementById("postcount_add").selectedIndex = "";
				document.getElementById("atotal").selectedIndex = "";
				document.getElementById("ltotal").selectedIndex = "";
				document.getElementById("shift_add").selectedIndex = "";
				document.getElementById("day_add").selectedIndex = "";
	...
	/////////////////////////////////////
// INITIALIZE THE DATE RANGE PICKER
tj.staffingdaterangepickerInit = function() {
