...
$(function () {
  ...
  $('#layout').w2layout({
    name: 'layout',
    panels: [
      {type: 'top', size: 50, style: pstyle, content: '<h1>Manage Candidates - ' + wus.surveyName + '</h1>'},
  ...
  $().w2toolbar({
    ...
    items: [
      ...
      {type: 'spacer'},
      {type: 'button', id: 'switchsms', caption: 'SMS for ' + wus.surveyName, icon: 'fa fa-list', hint: 'Switch to SMS View for these Candidates'},
    ...
    onClick: function (event) {
      switch (event.target) {
        ...
        case 'actions':
          ...
          break;
        case 'switchsms':
          window.location = 'smsview?sid=' + wus.surveyId;
          break;
        case 'refresh':
          w2ui['grid'].reset();
          //w2ui['grid'].reload();
