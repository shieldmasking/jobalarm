...
//echo "m ".$mobile;
	...
	if ($candidateId) {
		...
		$newcandidate = Config::get('db')->get_results("SELECT * from `candidate` WHERE `mobile`='{$mobile}' order by `id` desc");	
	...
	$insertdata = array(
		...
		$appid = Config::get('db')->get_results("SELECT * from `candidateApply` WHERE `candidateId` =$candidateId AND `accountId` =$accountId order by `id` desc");
		$applyId = $appid[0]['id'];
...
<div class="form-group">									
  ...
  <input type="radio" id="underageyes" name="age" value="No"> No <br>
</div>
<div class="form-group">									
...
<label><strong> How much job related experience do you have?</strong></label>							
<div class="row">
 <div class="col-md-3">
