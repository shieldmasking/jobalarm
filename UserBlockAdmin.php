...
$usernameData = Config::get('db')->get_results('
    SELECT userName, COUNT(userName) AS username FROM `job` GROUP BY userName HAVING (COUNT(userName)>1) ORDER BY username DESC limit 0,500
');
foreach($usernameData as $username) {
    $userNameList[$username['userName']] = array('username'=>$username['userName'],'count'=>$username['count'],'selected'=>0);
}

$dbData = Config::get('db')->get_results('select lower(username) as username from blocklist where blocked>0');

foreach($dbData as $username) {
    if (isset($userNameList[$username['username']])) {
        $userNameList[$username['username']]['selected'] = 1;
    } else {
        $query = "select count from job where userName='{$username['username']}'";
        $subData = Config::get('db')->get_results($query);
        $userNameList[$username['username']] = array('username'=>$username['username'],'count'=>(count($subData) > 0 ? $subData[0]['count'] : 0),'selected'=>1);
    }
}

function usernamesorter($a,$b) {
    return $b['count'] - $a['count'];
}
usort($userNameList,usernamesorter);
                    ...
                    <div class="col-md-12 col-sm-12">
                        ...
                        <div class="portlet light ">
                            ...
                            <div class="portlet-body" id="hastageditbox">
                              <select multiple="multiple" size="10" name="usernameeditor" id="usernameeditor">
                              <?php
                              foreach($userNameList as $username):
                              ?>                                
                                <option value="<?php echo $username['username']; ?>"<?php echo ($username['selected'] > 0) ? 'selected="selected"' : '' ?>><?php echo $username['username'].' ('.$username['count'].')'; ?></option>
                              <?php
                              endforeach;
                              ?>
                              </select>
                              <br />
                              <button id="save_usernames_btn" class="btn blue btn-default btn-block">Save</button>
                            </div>
                        </div>
                        <!-- END PORTLET-->
                    </div>
...
<![endif]-->
    ...
    <script src="inc/tweetedjobs-admin-usernames.js" type="text/javascript"></script>
    <script>
